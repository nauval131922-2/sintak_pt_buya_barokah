...
const COMBOS = Object.keys(EXPECTED) as SoftCoverOffsetComboId[];
// Master bawaan per file:
const FILE_P: Record<string, any> = {
  'Oliver-Oliver': { insheetCover: 100, insheetIsi: 100 },
  'Print-Oliver': { insheetCover: 10, insheetIsi: 100 },
  'Print-Print': { insheetCover: 10, insheetIsi: 7 },
};
...
function oracle(o: {
  ...
  const R = o.H > 0 ? o.H / P + K / O : 0;
  const Q = R * O * N;
  const [cvW, cvH] = o.combo === 'Oliver-Oliver' ? [65, 100] : [32.5, 48];
  const rimC = ((cvW * cvH) * o.p.gramaturCover) / 20000 * (o.p.tarifKertasCoverKg * (1 + o.p.upCoverPct / 100));
  const T = o.combo === 'Oliver-Oliver' ? (R / 500) * rimC : o.p.tarifPrintCoverA3 * R;
  const V = o.H > 0 ? o.p.tarifDesainCover : 0;
  const covOff = o.combo === 'Oliver-Oliver';
  ...
  const [AK, AL, AM] = o.combo === 'Print-Print' ? [2, 1, 4] : [4, 2, 16];
  const AN = o.hal / (AM / AL);
  const AN6 = Math.ceil(AN);
  const AO = ((o.H / AL) * AN + (AI / AL) * AN6) * AL;
  const AP = o.H > 0 ? ((o.H / AL) * AN + (AI / AL) * AN6) : 0;
  const [iaW, iaH] = o.combo === 'Print-Print' ? [29.7, 42] : o.wi === '1 Warna' ? [61, 86] : [65, 100];
  ...
  const isiOff = o.combo !== 'Print-Print';
  const AW = isiOff && o.H > 0 ? o.p.tarifPlateIsi * AX : 0;
  const BA = isiOff && o.H > 0 ? o.p.tarifCetakMinIsi * AX : 0;
  const BB = o.combo !== 'Print-Print' && (o.H + AI - 1000) * AX > 1 ? (o.H + AI - 1000) * AX : 0;
  const BD = o.combo === 'Print-Print' ? 700 * AO : BB * o.p.tarifDrekIsi + BA;
  ...
  if (cfg.jasaModel === 'UMR5') {
    ...
    const tgt = o.hal <= 20 ? 600 : o.hal <= 30 ? 550 : o.hal <= 40 ? 500 : o.hal <= 50 ? 450 : o.hal <= 60 ? 400 : o.hal <= 70 ? 350 : 300;
  ...
  const BY = full && o.H > 0 ? ((21 + 4) * (29.7 + 4)) * 500 * 2 : 0;
  const BZ = full ? o.H * (umrH / 1000) : 0;
  const bendOn = o.fin === 'Lem Bending,' || full;
  const CD = o.p.tarifBending * 29.7 * M * o.H;
...
react('plateIsi', OO, 700, { tarifPlateIsi: 50000 });
react('minIsi', OO, 700, { tarifCetakMinIsi: 100000 });
react('drekIsi', OO, 3000, { tarifDrekIsi: 50 });
...
react('jasaBuya', PP, 100, { tarifJasaPrintBuyaIsi: 800 });
