...
export default function SphInClient() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('sphIn_columnWidths');
      return saved ? JSON.parse(saved) : {
        id: 80, faktur: 220, tgl: 120, kd_supplier: 250, total: 180, username: 120, recid: 80
      };
    }
    return {};
  });
  ...
  useEffect(() => {
    localStorage.setItem('sphIn_columnWidths', JSON.stringify(columnWidths));
  }, [columnWidths]);
  ...
  useEffect(() => {
    setIsMounted(true);
    const hydratedPeriod = hydrateScraperPeriod({ stateKey: 'sphInState', periodKey: 'SphInClient_scrapedPeriod' });
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const res = await fetch(`/api/sph-in?${queryParams.toString()}`);
  ...
  const handleFetch = async () => {
    ...
    localStorage.setItem('sphInState', JSON.stringify({
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-sph-in?start=${chunk.start}&end=${chunk.end}&metaStart=${startStr}&metaEnd=${endStr}&silent=true`);
        if (res.ok) {
    ...
    try {
      ...
      if (successCount > 0) {
        persistScraperPeriod({ stateKey: 'sphInState', periodKey: 'SphInClient_scrapedPeriod' }, startDate, endDate);
        setRefreshKey(prev => prev + 1);
        localStorage.setItem('sintak_data_updated', Date.now().toString());
        fetch('/api/activity-log', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action_type: 'SCRAPE', table_name: 'sph_in', message: `Scrape SPH In berhasil: ${totalScraped} baris (${startStr} - ${endStr}).`, raw_data: JSON.stringify({ total: totalScraped, start: startStr, end: endStr }) }),
        }).catch(() => {});
        setDialog({ isOpen: true, type: 'success', title: 'Berhasil', message: `Berhasil menarik ${totalScraped} data SPH Masuk.` });
      }
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      header: 'Faktur SPPH',
      size: 220,
      cell: ({ getValue, row }: any) => <span className={`font-bold tracking-tight transition-colors ${row.getIsSelected() ? 'text-green-600' : 'text-gray-500'}`}>{String(getValue() || '–')}</span>
    },
    {
      accessorKey: 'total',
      header: 'Nilai SPH',
    ...
    },
    {
      ...
      size: 100,
      cell: ({ getValue }: any) => (
        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold leading-none border ${
          getValue() === '1' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gray-50 text-gray-400 border-gray-100'
        }`}>
          {getValue() === '1' ? 'ACTIVE' : 'INACTIVE'}
        </span>
      )
    },
    {
      ...
      header: 'Status PO',
      size: 150,
      cell: ({ getValue, row }: any) => <div className={`flex items-center text-[12px] font-bold ${row.getIsSelected() ? 'text-green-600' : 'text-gray-400'}`} dangerouslySetInnerHTML={{ __html: String(getValue() || '–') }} />
    },
    { 
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-6 animate-in fade-in duration-700 overflow-hidden">
      ...
      {error && (
        <div className="p-4 bg-rose-50 text-rose-600 border border-rose-100 rounded-lg text-[11px] font-bold flex items-start gap-3 animate-in slide-in-from-top-2 z-20 shadow-sm shadow-rose-900/5">
      ...
      <div className="flex-1 flex flex-col gap-3 overflow-hidden min-h-0 relative">
        <div className="flex flex-col gap-4 shrink-0 px-1">
          <div className="flex items-center justify-between gap-4 min-h-[32px]">
            <ScrapingHeader title="Hasil Scrapping SPH Masuk" lastUpdated={lastUpdated} scrapedPeriod={scrapedPeriod} activityLogTable="sph_in" />
          ...
          <SearchAndReload searchQuery={searchQuery} setSearchQuery={setSearchQuery} onReload={() => setRefreshKey(prev => prev + 1)} loading={loading} placeholder="Cari faktur, supplier, SPPH..." />
        </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden relative">
          ...
          <TableFooter totalCount={totalCount} currentCount={data?.length || 0} label="SPH Masuk" selectedCount={selectedIds.size} onClearSelection={clearSelection} loadTime={loadTime} />
        </div>
      </div>
