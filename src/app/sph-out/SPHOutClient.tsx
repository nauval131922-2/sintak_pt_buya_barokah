...
export default function SphOutClient() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('sphOut_columnWidths');
      return saved ? JSON.parse(saved) : {
        id: 80, faktur: 220, tgl: 120, kd_pelanggan: 250, total: 180, username: 120, recid: 80
      };
    }
    return {};
  });
  ...
  useEffect(() => {
    localStorage.setItem('sphOut_columnWidths', JSON.stringify(columnWidths));
  }, [columnWidths]);
  ...
  useEffect(() => {
    setIsMounted(true);
    const hydratedPeriod = hydrateScraperPeriod({ stateKey: 'sphOutState', periodKey: 'SphOutClient_scrapedPeriod' });
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const res = await fetch(`/api/sph-out?${queryParams.toString()}`);
  ...
  const handleFetch = async () => {
    ...
    localStorage.setItem('sphOutState', JSON.stringify({
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-sph-out?start=${chunk.start}&end=${chunk.end}&metaStart=${startStr}&metaEnd=${endStr}&silent=true`);
        if (res.ok) {
    ...
    try {
      ...
      if (successCount > 0) {
        persistScraperPeriod({ stateKey: 'sphOutState', periodKey: 'SphOutClient_scrapedPeriod' }, startDate, endDate);
        setRefreshKey(prev => prev + 1);
        localStorage.setItem('sintak_data_updated', Date.now().toString());
        fetch('/api/activity-log', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action_type: 'SCRAPE', table_name: 'sph_out', message: `Scrape SPH Out berhasil: ${totalScraped} baris (${startStr} - ${endStr}).`, raw_data: JSON.stringify({ total: totalScraped, start: startStr, end: endStr }) }),
        }).catch(() => {});
        setDialog({ isOpen: true, type: 'success', title: 'Berhasil', message: `Berhasil menarik ${totalScraped} data SPH Keluar.` });
      }
    ...
    <div className="flex-1 min-h-0 flex flex-col gap-6 animate-in fade-in duration-500 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-3 overflow-hidden min-h-0 relative">
        <div className="flex flex-col gap-4 shrink-0 px-1">
          ...
          <SearchAndReload searchQuery={searchQuery} setSearchQuery={setSearchQuery} onReload={() => setRefreshKey(prev => prev + 1)} loading={loading} placeholder="Cari nomor faktur, pelanggan, atau keterangan..." />
        </div>

        <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden relative">
          <DataTable columns={columns} data={data || []} isLoading={loading} onScroll={handleScroll} selectedIds={selectedIds} onRowClick={handleRowClick} columnWidths={columnWidths} onColumnWidthChange={setColumnWidths} rowHeight="h-11" />
          <TableFooter totalCount={totalCount} currentCount={data?.length || 0} label="SPH Keluar" selectedCount={selectedIds.size} onClearSelection={clearSelection} loadTime={loadTime} />
        </div>
      </div>
