...
export async function GET(req: NextRequest) {
  try {
    ...
    let query = `SELECT * FROM pelunasan_hutang WHERE 1=1`;
    let countQuery = `SELECT COUNT(*) as total FROM pelunasan_hutang WHERE 1=1`;
    const params: any[] = [];

    if (search) {
      const searchPattern = `%${search}%`;
      const searchSql = ` AND (faktur LIKE ? OR kd_supplier LIKE ? OR faktur_pb LIKE ? OR keterangan LIKE ?)`;
      query += searchSql;
    ...
    const metadataResults = await db.batch([
      { sql: `SELECT value FROM system_settings WHERE key = 'last_scrape_pelunasan_hutang'`, args: [] },
      { sql: `SELECT value FROM system_settings WHERE key = ?`, args: [getScrapedPeriodSettingKey('last_scrape_pelunasan_hutang')] },
      { sql: `SELECT strftime('%Y-%m-%dT%H:%M:%SZ', MAX(created_at)) as lastUpdated FROM pelunasan_hutang`, args: [] }
    ], "read");
    ...
    console.error('API Error (pelunasan-hutang):', error);
