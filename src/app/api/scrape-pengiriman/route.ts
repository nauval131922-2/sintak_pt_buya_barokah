  ...
  try {
    ...
    await db.batch([
      ...
      },
      {
        ...
        args: [getScrapedPeriodSettingKey('last_scrape_pengiriman'), encodeScrapedPeriod({ start: metaStart, end: metaEnd })]
      }
    ], "write");
    ...
    if (!silent) {
      await logActivity(
        'SCRAPE',
        'pengiriman',
        `Scrape pengiriman berhasil: ${filteredRows.length} baris (${metaStart} - ${metaEnd}).`,
