...
export async function GET(request: NextRequest) {
  try {
    ...
    const dataUrl = BASE_URL + "v1/pb/trspph_out/gr1?request=" + reqJson;
    ...
    for (const record of allRecords) {
      batchOps.push({
        sql: `
          INSERT INTO spph_out (
            faktur, tgl, faktur_pr, faktur_prd, kd_gudang, kd_cabang, 
            kd_supplier, status, keterangan, faktur_sph, raw_data
          )
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    await db.batch([
      {
        ...
        `,
        args: ['last_scrape_spph_out', lastUpdated]
      },
      {
        sql: `
          INSERT INTO system_settings (key, value, updated_at)
          ...
          ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = CURRENT_TIMESTAMP
        `,
        args: [getScrapedPeriodSettingKey('last_scrape_spph_out'), encodeScrapedPeriod({ start: metaStart, end: metaEnd })]
      }
    ], "write");
    ...
    if (!silent) {
      await logActivity(
        'SCRAPE',
        'spph_out',
        `Scrape SPPH Out berhasil: ${allRecords.length} baris (${metaStart} - ${metaEnd}).`,
    ...
    console.error("Scrape SPPH Out Error:", error);
