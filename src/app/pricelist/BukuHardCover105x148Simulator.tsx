...
const DRAFT_KEY = 'sintak_buku_hard_cover_105x148_draft';
...
import {
  BukuHardCover105x148MasterParams,
  BukuHardCover105x148SimulatorInput,
  BukuHardCover105x148SimulatorResult,
  BukuHardCover105x148FinishingOption,
  BUKU_HARD_COVER_105X148_TIERS,
  calculateBukuHardCover105x148Simulator,
} from '@/lib/buku-hard-cover-105x148-calculator';

export type SavedBukuHardCover105x148SimulationItem = {
  id: string;
  savedAt: string;
  title: string;
  oplah: number;
  data: BukuHardCover105x148SimulatorResult;
  paramsSnapshot?: any;
};

const FINISHING_OPTIONS: BukuHardCover105x148FinishingOption[] = [
  'Tanpa Laminasi',
  'Laminasi Glossy',
  'Laminasi Doff',
];
...
interface Props {
  customParams: BukuHardCover105x148MasterParams;
...
export default function BukuHardCover105x148Simulator({
  customParams,
  onOpenMasterParam,
  activeSimulationId,
  ...
  const [finishing, setFinishing] = useState<BukuHardCover105x148FinishingOption>(() => draftVal('finishing', 'Tanpa Laminasi'));
  ...
  const result = useMemo<BukuHardCover105x148SimulatorResult>(() => {
    return calculateBukuHardCover105x148Simulator(
      {
        oplah,
        finishing,
        opsiFoil,
  ...
  }, [oplah, finishing, opsiFoil, marginPct, negoDiskonPct, customParams]);

  useEffect(() => {
    if (activeSimulationId) {
      try {
        const raw = localStorage.getItem('sintak_saved_buku_hard_cover_105x148_simulations');
        if (raw) {
          const list: SavedBukuHardCover105x148SimulationItem[] = JSON.parse(raw);
          ...
          if (found) {
            ...
            setOpsiFoil(found.data.input.opsiFoil ?? false);
      ...
      try {
        ...
        if (rawDraft) {
          ...
          if (d.opsiFoil !== undefined) setOpsiFoil(Boolean(d.opsiFoil));
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(DRAFT_KEY, JSON.stringify({ oplah, finishing, opsiFoil, marginPct, negoDiskonPct }));
    } catch { /* abaikan */ }
  }, [oplah, finishing, opsiFoil, marginPct, negoDiskonPct, activeSimulationId]);

  const handleSaveSimulation = () => {
    try {
      const existingRaw = localStorage.getItem('sintak_saved_buku_hard_cover_105x148_simulations');
      const list: SavedBukuHardCover105x148SimulationItem[] = existingRaw ? JSON.parse(existingRaw) : [];

      const title =
        activeSimulationTitle ||
        `Buku Hard Cover 10,5×14,8 • ${oplah.toLocaleString('id-ID')} pcs • ${finishing}${opsiFoil ? ' + Foil' : ''}`;
      ...
      if (activeSimulationId) {
        ...
        if (targetItem) saveCalculationToDb({ ...targetItem, category: 'Buku Hard Cover 10,5×14,8' });
        ...
        const newItem: SavedBukuHardCover105x148SimulationItem = {
          id: `bhc_105x148_${Date.now()}`,
          savedAt: new Date().toISOString(),
          title,
          oplah,
          data: result,
        ...
        localStorage.setItem('sintak_saved_buku_hard_cover_105x148_simulations', JSON.stringify(list));
    saveCalculationToDb({ ...newItem, category: 'Buku Hard Cover 10,5×14,8' });
  ...
  const handleSaveAsNew = () => {
    try {
      const existingRaw = localStorage.getItem('sintak_saved_buku_hard_cover_105x148_simulations');
      const list: SavedBukuHardCover105x148SimulationItem[] = existingRaw ? JSON.parse(existingRaw) : [];
      const title = `Buku Hard Cover 10,5×14,8 • ${oplah.toLocaleString("id-ID")} pcs • ${finishing}${opsiFoil ? " + Foil" : ""}`;
      const newItem: SavedBukuHardCover105x148SimulationItem = {
        id: `bhc_105x148_${Date.now()}`,
        savedAt: new Date().toISOString(),
        title,
  ...
  const handleCopyWhatsApp = () => {
    ...
    const text = `*PENAWARAN BUKU HARD COVER 10,5×14,8 CM (A6)*
*PT Buya Barokah*
━━━━━━━━━━━━━━━━━━━━
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-slate-800 text-sm sm:text-base">
                Simulator &amp; Kalkulator Buku Hard Cover 10,5×14,8 cm
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Kalkulasi instan HPP, harga jual, dan batas nego untuk Buku Hard Cover A6 (100 Halaman).
            </p>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  value={BUKU_HARD_COVER_105X148_TIERS.includes(oplah) ? oplah : 'custom'}
                  onChange={(e) => {
                ...
                >
                  {BUKU_HARD_COVER_105X148_TIERS.map((t) => (
                    <option key={t} value={t}>
                      {t.toLocaleString('id-ID')} pcs
                    </option>
                  ))}
                  {!BUKU_HARD_COVER_105X148_TIERS.includes(oplah) && (
              ...
              <div className="grid grid-cols-3 gap-2">
                {FINISHING_OPTIONS.map((f) => (
                  <button
                    ...
                    className={`py-2 px-2 rounded-lg border text-xs font-bold transition-all text-center cursor-pointer ${
                      finishing === f
            ...
            <div>
              ...
              <label className="flex items-center gap-2 p-2.5 rounded-lg border border-slate-200 bg-slate-50/50 hover:bg-slate-50 cursor-pointer text-xs">
                ...
                />
                <span className="font-bold text-slate-700">Foil Emas Cover (+Rp 450/pcs)</span>
              </label>
            </div>
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between text-xs text-slate-700">
            ...
            <span className="text-[11px] text-slate-500">
              Hard Cover Art Paper 150 + Board • Isi 100 Hal HVS 70
            </span>
          </div>
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base">Panduan Simulator Buku Hard Cover 10,5×14,8 cm</h3>
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs text-slate-600 leading-relaxed">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <p>
                  Pilih oplah pesanan dari dropdown tier (50 s/d 5.000 pcs) atau ketik angka custom.
                  Sistem otomatis mengelompokkan ke mesin cetak yang sesuai (Print, Ryobi, Oliver, atau Heidelberg SM).
                </p>
              </div>
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <p>
                  Tersedia opsi Laminasi Glossy/Doff dan Foil Emas pada Hard Cover.
                  Biaya finishing otomatis ditambahkan ke total HPP dan harga jual per pcs.
                </p>
              </div>
