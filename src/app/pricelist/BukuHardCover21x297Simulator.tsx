...
const DRAFT_KEY = 'sintak_buku_hard_cover_21x297_draft';
...
export default function BukuHardCover21x297Simulator({
  customParams,
  onOpenMasterParam,
  activeSimulationId,
  ...
  const [finishing, setFinishing] = useState<BukuHardCover21x297FinishingOption>(() => draftVal('finishing', 'Tanpa Laminasi'));
  ...
  const result = useMemo<BukuHardCover21x297SimulatorResult>(() => {
    return calculateBukuHardCover21x297Simulator(
      {
        oplah,
        finishing,
        opsiFoil,
  ...
  useEffect(() => {
    if (activeSimulationId) {
      try {
        const raw = localStorage.getItem('sintak_saved_buku_hard_cover_21x297_simulations');
        if (raw) {
          const list: SavedBukuHardCover21x297SimulationItem[] = JSON.parse(raw);
  ...
  const handleSaveSimulation = () => {
    try {
      const existingRaw = localStorage.getItem('sintak_saved_buku_hard_cover_21x297_simulations');
      const list: SavedBukuHardCover21x297SimulationItem[] = existingRaw ? JSON.parse(existingRaw) : [];

      const title =
        activeSimulationTitle ||
        `Buku Hard Cover 21×29,7 • ${oplah.toLocaleString('id-ID')} pcs • ${finishing}${opsiFoil ? ' + Foil' : ''}`;
      ...
      if (activeSimulationId) {
        ...
        localStorage.setItem('sintak_saved_buku_hard_cover_21x297_simulations', JSON.stringify(list));
    saveCalculationToDb({ ...newItem, category: 'Buku Hard Cover 21×29,7' });
  ...
  const handleSaveAsNew = () => {
    try {
      const existingRaw = localStorage.getItem('sintak_saved_buku_hard_cover_21x297_simulations');
      const list: SavedBukuHardCover21x297SimulationItem[] = existingRaw ? JSON.parse(existingRaw) : [];
      const title = `Buku Hard Cover 21×29,7 • ${oplah.toLocaleString("id-ID")} pcs • ${finishing}${opsiFoil ? " + Foil" : ""}`;
      const newItem: SavedBukuHardCover21x297SimulationItem = {
        id: `bhc_21x297_${Date.now()}`,
        savedAt: new Date().toISOString(),
        title,
  ...
  const handleCopyWhatsApp = () => {
    ...
    const text = `*PENAWARAN BUKU HARD COVER 21×29,7 CM (A4)*
*PT Buya Barokah*
━━━━━━━━━━━━━━━━━━━━
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-slate-800 text-sm sm:text-base">
                Simulator &amp; Kalkulator Buku Hard Cover 21×29,7 cm (A4)
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Kalkulasi instan HPP, harga jual, dan batas nego untuk Buku Hard Cover A4 (100 Halaman).
            </p>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  value={BUKU_HARD_COVER_21X297_TIERS.includes(oplah) ? oplah : 'custom'}
                  onChange={(e) => {
                ...
                >
                  {BUKU_HARD_COVER_21X297_TIERS.map((t) => (
                    <option key={t} value={t}>
                      {t.toLocaleString('id-ID')} pcs
                    </option>
                  ))}
                  {!BUKU_HARD_COVER_21X297_TIERS.includes(oplah) && (
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between text-xs text-slate-700">
            ...
            <span className="text-[11px] text-slate-500">
              Hard Cover Art Paper 150 + Board No. 30 • Isi 100 Hal HVS 70 (A4)
            </span>
          </div>
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base">Panduan Simulator Buku Hard Cover 21×29,7 cm (A4)</h3>
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs text-slate-600 leading-relaxed">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <p>
                  Pilih oplah pesanan dari dropdown tier (250 s/d 5.000 pcs) atau ketik angka custom.
                  Sistem otomatis mengelompokkan ke mesin cetak yang sesuai (Oliver atau Heidelberg SM).
                </p>
              </div>
