...
const VARIAN_LIST: BukuTabunganSecurityVarianType[] = ['24 Hal', '32 Hal', '48 Hal'];
...
export default function BukuTabunganSecurityMatrixView({
  ...
  const [selectedVarianFilter, setSelectedVarianFilter] = useState<BukuTabunganSecurityVarianType | 'ALL'>('ALL');
  ...
  const calc = (oplah: number, varian: BukuTabunganSecurityVarianType) =>
    calculateBukuTabunganSecurityHpp(
  ...
  const matrixData = useMemo(() => {
    ...
    return BUKU_TABUNGAN_SECURITY_TIERS.map((oplah) => {
    ...
    }).filter(Boolean) as { oplah: number; cols: { varian: BukuTabunganSecurityVarianType; hpp: number; jual: number; nego: number; totalJual: number }[] }[];
  }, [customParams, searchTerm, selectedVarianFilter]);

  const flatTableRows = useMemo(() => {
    const list: {
      oplah: number; varian: BukuTabunganSecurityVarianType; hpp: number; jual: number; nego: number; totalJual: number; margin: number;
    }[] = [];

    const varians = selectedVarianFilter === 'ALL' ? VARIAN_LIST : [selectedVarianFilter];

    varians.forEach((varian) => {
      BUKU_TABUNGAN_SECURITY_TIERS.forEach((oplah) => {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm sm:text-base font-bold text-emerald-950 tracking-tight">
              Pricelist Matriks Buku Tabungan Security
            </h2>
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Tabel perbandingan HPP &amp; harga jual Buku Tabungan 9×14,5 cm Security 24/32/48 Hal per oplah &amp; varian (margin 30%, nego 5%, Ivory 260 + HVS 70, foil emas + numbering, jahit+pound+security).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center gap-3 text-xs">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
          ...
          <select
            ...
            onChange={(e) => setSelectedVarianFilter(e.target.value as BukuTabunganSecurityVarianType | 'ALL')}
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-gray-800 tracking-tight">Buku Tabungan 9×14,5 cm — Security · Ivory 260 + HVS 70 + Foil + Numbering</h3>
                </div>
                ...
                <div key={varian} className="bg-white rounded-xl border border-gray-200 shadow-xs overflow-hidden">
                  <div className="bg-teal-50/70 px-4 py-2 border-b border-teal-100 flex items-center justify-between">
                    <span className="text-[11px] font-bold text-teal-900 tracking-wider uppercase flex items-center gap-1.5">
                      ...
                      Varian: {varian} — {BUKU_TABUNGAN_SECURITY_CONFIG[varian].jumlahHalaman} Hal ({BUKU_TABUNGAN_SECURITY_CONFIG[varian].jumlahLbr} lbr) · {BUKU_TABUNGAN_SECURITY_CONFIG[varian].w}×{BUKU_TABUNGAN_SECURITY_CONFIG[varian].h} cm · {BUKU_TABUNGAN_SECURITY_CONFIG[varian].coverPcsPerA3} cov/A3+ · {BUKU_TABUNGAN_SECURITY_CONFIG[varian].leavesPerA3} isi/A3+ · Security
                    </span>
                  </div>
