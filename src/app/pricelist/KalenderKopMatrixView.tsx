...
export default function KalenderKopMatrixView({
  customParams,
  ...
  const [selectedVarian, setSelectedVarian] = useState<string>('ALL');
  const [marginPct, setMarginPct] = useState<number>(customParams.marginDefaultPct ?? 0);
  const [negoPct, setNegoPct] = useState<number>(customParams.negoDefaultPct ?? 4);
  const [searchTerm, setSearchTerm] = useState<string>('');

  const matrixData = useMemo(() => {
    return recalculateKalenderKopMatrix(
      customParams,
      marginPct,
      negoPct
    );
  }, [customParams, marginPct, negoPct]);

  const filteredData = useMemo(() => {
    return matrixData.filter((item) => {
      if (selectedVarian !== 'ALL' && item.varian !== selectedVarian) {
        return false;
      }
  ...
  }, [matrixData, selectedVarian, searchTerm]);
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-slate-800 text-sm sm:text-base">
                Pricelist Matriks Kalender Kop
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Matriks harga 4 varian warna Kalender Kop across tier oplah terhitung instan.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-lg border border-slate-200">
            <span className="text-[11px] font-bold text-slate-500 pl-1.5">Varian:</span>
            <button
              type="button"
              onClick={() => setSelectedVarian('ALL')}
      ...
      {viewMode === 'matrix' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3.5">
          {filteredData.map((item, idx) => (
            <div
              key={`${item.varian}-${item.oplah}-${idx}`}
            ...
            >
              <div>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-black text-slate-800">
                    {item.oplah.toLocaleString('id-ID')} eks
                  </span>
              ...
              <div className="space-y-1 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                ...
                <div className="flex items-center justify-between text-[11px] border-t border-slate-200/60 pt-1">
                  <span className="text-slate-400">HPP / eks</span>
            ...
            <table className="w-full text-xs text-left border-collapse">
              <thead>
                <tr className="bg-slate-100 text-slate-700 border-b border-slate-200 text-[11px]">
                  ...
                  <th className="py-2.5 px-3 font-bold">Varian Kalender Kop</th>
                  <th className="py-2.5 px-3 font-bold">Oplah Pesanan</th>
                  <th className="py-2.5 px-3 font-bold text-right">HPP / eks</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.map((row, idx) => (
                  <tr key={`${row.varian}-${row.oplah}-${idx}`} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-2.5 px-3 font-black text-slate-800">
                      Kalender Kop {row.varian}
                    </td>
                    <td className="py-2.5 px-3 font-bold text-slate-800">
                      {row.oplah.toLocaleString('id-ID')} eks
                    </td>
