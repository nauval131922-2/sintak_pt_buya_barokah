...
export default function BukuTulisSimulator({
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('sintak_saved_buku_tulis_simulations');
      if (raw) {
        const list: SavedBukuTulisSimulationItem[] = JSON.parse(raw);
  ...
  const defaultTitle = () =>
    `Buku Tulis ${ukuran} 72 Hal (${oplah} pcs) ${opsiLaminasi ? '+Laminasi' : ''}`;

  const handleSaveSimulation = () => {
    const title = simulationTitle.trim() || defaultTitle();
    const newItem: SavedBukuTulisSimulationItem = {
      id: 'buku_tulis_' + Date.now(),
      title,
    ...
    try {
      ...
      console.error('Failed to save buku tulis simulation:', e);
    }
    setSimulationTitle('');
  ...
  const handleUpdateSavedSimulation = () => {
    ...
    try {
      ...
      console.error('Failed to update buku tulis simulation:', e);
    }
  ...
  const handleCopyQuote = () => {
    ...
    const text =
      `*PENAWARAN BUKU TULIS 72 HAL*\n` +
      `*PT Buya Barokah*\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `• *Produk*: Buku Tulis 72 Hal Soft Cover\n` +
      `• *Ukuran*: ${ukuran} cm (tertutup) – ${cfg.description}\n` +
      `• *Kuantitas*: ${oplah} pcs\n` +
      `• *Finishing*: ${opsiLaminasi ? 'Laminasi Glossy' : 'Tanpa Laminasi'}, ${opsiSisir ? 'Sisir + Packing' : 'Standar'}\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      ...
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `_Harga belum termasuk PPN. 1 muka cover full colour, isi HVS 70 gsm 1 warna bolak-balik._`;

    navigator.clipboard.writeText(text);
    setCopiedQuote(true);
    toast.success('Penawaran harga Buku Tulis berhasil disalin ke WhatsApp clipboard!');
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-sm sm:text-base text-emerald-950 flex items-center gap-2">
              Simulator &amp; Kalkulator Buku Tulis
            ...
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Hitung HPP, harga penawaran, dan estimasi profit buku tulis 72 hal Soft Cover (15,5×21 & 16×21 cm).
            </p>
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-300 rounded-xl p-3 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-in fade-in duration-200">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={() => {
                if (setCustomParams) setCustomParams(DEFAULT_BUKU_TULIS_PARAMS);
                toast.success('Parameter Buku Tulis dikembalikan ke tarif Master Standar!');
              }}
        ...
        <div className="lg:col-span-5 space-y-5">
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
              ...
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Input Spesifikasi Buku Tulis</h3>
            </div>

            {/* Ukuran */}
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                Ukuran Buku (tertutup)
              </label>
              <div className="grid grid-cols-2 gap-2">
                {UKURAN_OPTIONS.map((u) => (
                  ...
                  >
                    <Book size={14} className={ukuran === u ? 'text-white' : 'text-slate-500'} />
                    <span>{u} cm</span>
                    <span className={`text-[10px] ${ukuran === u ? 'text-emerald-100' : 'text-slate-400'}`}>72 hal</span>
                  </button>
                ))}
              </div>
              <p className="text-[11px] text-slate-500 mt-1.5 italic">{BUKU_TULIS_CONFIG[ukuran].description}</p>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  value={BUKU_TULIS_TIERS.includes(oplah) ? oplah : 'custom'}
                  onChange={(e) => {
                ...
                >
                  {BUKU_TULIS_TIERS.map((t) => (
                    <option key={t} value={t}>{t.toLocaleString('id-ID')} pcs</option>
                  ))}
                  {!BUKU_TULIS_TIERS.includes(oplah) && <option value="custom">{oplah.toLocaleString('id-ID')} pcs (custom)</option>}
                </select>
                <input
                  type="number"
                  min={1}
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="flex items-center gap-2 p-2.5 rounded-lg border border-slate-200 bg-slate-50/50 hover:bg-slate-50 cursor-pointer text-xs">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-400">Cover tahan air</span>
                  </div>
                </label>
                <label className="flex items-center gap-2 p-2.5 rounded-lg border border-slate-200 bg-slate-50/50 hover:bg-slate-50 cursor-pointer text-xs">
                  <input
                  ...
                  />
                  <div>
                    <span className="font-semibold text-slate-800 block text-[11px]">Sisir + Packing</span>
                    <span className="text-[10px] text-slate-400">Rapih + kardus</span>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="px-4 py-3 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                  Rincian Estimasi Komponen Biaya Buku Tulis
                </h4>
              </div>
              <span className="text-[11px] font-bold text-slate-500">
                {oplah.toLocaleString('id-ID')} pcs · {ukuran} · 72 hal
              </span>
            </div>
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold tracking-tight">Panduan Simulator Buku Tulis 72 Hal</h3>
                  <p className="text-xs text-emerald-200/90 mt-0.5">
                    Alur perhitungan berbasis oplah pcs, 72 hal (18 lembar), AC 230 gsm + HVS 70 gsm, laminasi & finishing staples tengah
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-xs text-slate-700 leading-relaxed">
              <div className="space-y-3">
                <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                  ...
                  Langkah Menggunakan Simulator Buku Tulis
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  {[
                    ['1. Ukuran & Oplah', 'Pilih 15,5×21 atau 16×21 cm (tertutup) dan tentukan oplah 20–10000 pcs via dropdown atau custom input.'],
                    ['2. Finishing', 'Aktifkan Laminasi Glossy cover & Sisir/Packing sesuai kebutuhan jilid staples tengah.'],
                    ['3. Margin & Nego', 'Atur margin profit (default 20%) & batas nego (default 4%), harga otomatis terhitung per pcs & total.'],
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2.5">
                <h5 className="font-bold text-slate-900 text-xs uppercase tracking-wider flex items-center gap-2">
                  ...
                  Struktur Biaya Produksi Buku Tulis
                </h5>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px]">
                  <div className="p-2.5 bg-white rounded border border-emerald-100 space-y-1">
                    ...
                    <p className="text-slate-600 leading-snug">
                      Cover AC 230 gsm: Rp 16.400/kg + up 5%, 4 warna 1 muka @ Rp 2.500/A3+ (Print Inter). Isi HVS 70 gsm: Rp 15.700/kg + up 3%, 1 warna bolak-balik @ Rp 2.000/A3+ (Ryobi ≤500, Oliver &gt;500).
                    </p>
                  </div>
                  <div className="p-2.5 bg-white rounded border border-blue-100 space-y-1">
                    <span className="font-bold text-blue-900 block">Finishing &amp; Desain:</span>
                    <p className="text-slate-600 leading-snug">
                      Laminasi glossy Rp 0,35/cm² (min 50k), susun+staples+sisir per pcs, packing kardus+lakban per order, desain cover Rp 20.000 + isi Rp 2.500/hal.
                    </p>
                  </div>
                </div>
              </div>
