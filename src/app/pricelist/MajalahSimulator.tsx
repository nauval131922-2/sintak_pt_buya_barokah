...
import {
  ...
  MajalahFinishingOption,
  MajalahJilidOption,
  MAJALAH_TIERS,
  calculateMajalahSimulator,
} from '@/lib/majalah-calculator';

export type SavedMajalahSimulationItem = {
  id: string;
  savedAt: string;
  title: string;
  oplah: number;
...
const JILID_OPTIONS: MajalahJilidOption[] = [
  'Staples Tengah',
  'Lem Bending',
];
...
export default function MajalahSimulator({
  customParams,
  onOpenMasterParam,
  activeSimulationId,
  ...
  const result = useMemo<MajalahSimulatorResult>(() => {
    return calculateMajalahSimulator(
      {
        oplah,
        finishing,
        jilid,
  ...
  useEffect(() => {
    if (activeSimulationId) {
      try {
        const raw = localStorage.getItem('sintak_saved_majalah_simulations');
        if (raw) {
          const list: SavedMajalahSimulationItem[] = JSON.parse(raw);
  ...
  const handleSaveSimulation = () => {
    try {
      ...
      const title =
        activeSimulationTitle ||
        `Majalah 14,5×20,25 • ${oplah.toLocaleString('id-ID')} pcs • ${finishing}`;
  ...
  const handleCopyWhatsApp = () => {
    ...
    const text = `*PENAWARAN MAJALAH 14,5×20,25 CM*
*PT Buya Barokah*
━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━
_Harga belum termasuk PPN. Hasil cetak tajam, warna akurat & presisi._`;
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-slate-800 text-sm sm:text-base">
                Simulator &amp; Kalkulator Majalah 14,5×20,25 cm
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Kalkulasi instan HPP, harga jual, dan batas nego untuk Majalah Full Colour 32 Halaman.
            </p>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  value={MAJALAH_TIERS.includes(oplah) ? oplah : 'custom'}
                  onChange={(e) => {
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between text-xs text-slate-700">
            ...
            <span className="text-[11px] text-slate-500">
              Cover AC 230 • Isi AP 120 FC (32 Hal)
            </span>
          </div>
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base">Panduan Simulator Majalah 14,5×20,25 cm</h3>
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs text-slate-600 leading-relaxed">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <p>
                  ...
                  Sistem otomatis mengarahkan ke alur mesin cetak: Print-Print (≤200 pcs), Print-Oliver (250–500 pcs), atau Oliver-Oliver (≥600 pcs).
                </p>
              </div>
