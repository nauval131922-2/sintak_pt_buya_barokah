...
import {
  ...
  TabunganSecurityMesinCover,
  TabunganSecurityMesinIsi,
  TabunganSecurityFinishing,
  TABUNGAN_SECURITY_MESIN_COVER,
  TABUNGAN_SECURITY_MESIN_ISI,
  TABUNGAN_SECURITY_FINISHING,
  TABUNGAN_SECURITY_GRAMATUR_COVER,
  TABUNGAN_SECURITY_GRAMATUR_ISI,
...
const DRAFT_KEY = 'sintak_tabungan_security_draft';
...
export default function BukuTabunganSecuritySimulator({
  ...
  const [mesinCover, setMesinCover] = useState<TabunganSecurityMesinCover>(() => draftVal('mesinCover', 'Otomatis'));
  ...
  const [mesinIsi, setMesinIsi] = useState<TabunganSecurityMesinIsi>(() => draftVal('mesinIsi', 'Otomatis'));
  ...
  const [finishing, setFinishing] = useState<TabunganSecurityFinishing>(() => draftVal('finishing', 'Laminasi Glossy,'));
  ...
  const applyInput = (inp: BukuTabunganSecuritySimulatorInput) => {
  ...
  const input: BukuTabunganSecuritySimulatorInput = useMemo(() => ({
    oplahPcs, jumlahHalaman, mukaCover, warnaCover, mesinCover, bahanCover, gramaturCover,
    warnaIsi, mesinIsi, bahanIsi, gramaturIsi, finishing, jahitAktif, pisauPoundAktif,
    jasaPoundAktif, sisirAktif, kardusAktif, insheetCover, insheetIsi, marginPct,
  ...
  const result = useMemo(() => calculateBukuTabunganSecurityHpp(input, customParams), [input, customParams]);
  void setCustomParams;

  const defaultTitle = () => `Buku Tabungan Security ${oplahPcs}pcs ${result.mesinIsiEfektif}`;

  const handleSaveSimulation = () => {
    const title = simulationTitle.trim() || defaultTitle();
    const newItem: SavedBukuTabunganSecuritySimulationItem = {
      id: 'buku_tabungan_security_' + Date.now(), title, savedAt: new Date().toISOString(),
    ...
    } catch (e) { console.error('Failed to save buku tabungan security simulation:', e); }
    setSimulationTitle('');
  ...
  const handleUpdateSavedSimulation = () => {
    ...
    } catch (e) { console.error('Failed to update buku tabungan security simulation:', e); }
  ...
  const handleCopyQuote = () => {
    ...
    const text =
      ...
      `*PT Buya Barokah*\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `• *Produk*: Buku Tabungan Security 9×14,5 cm ${jumlahHalaman} Hal\n` +
    ...
    setCopiedQuote(true);
    toast.success('Penawaran harga Buku Tabungan Security berhasil disalin!');
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-sm sm:text-base text-emerald-950 flex items-center gap-2">
              ...
              <span className="text-[11px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-semibold border border-emerald-200">Katalog 15</span>
            </h3>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select value={BUKU_TABUNGAN_SECURITY_TIERS.includes(oplahPcs) ? oplahPcs : 'custom'}
                  onChange={(e) => { const v = e.target.value; if (v !== 'custom') setOplahPcs(Number(v)); }}
                  className={`${numInput} cursor-pointer`}>
                  {BUKU_TABUNGAN_SECURITY_TIERS.map((t) => <option key={t} value={t}>{t.toLocaleString('id-ID')} pcs</option>)}
                  {!BUKU_TABUNGAN_SECURITY_TIERS.includes(oplahPcs) && <option value="custom">{oplahPcs.toLocaleString('id-ID')} pcs (custom)</option>}
                </select>
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-1.5">
                {TABUNGAN_SECURITY_MESIN_COVER.map((m) => (
              ...
              <div className="grid grid-cols-2 gap-2 mt-1.5">
                ...
                <select value={gramaturCover} onChange={(e) => setGramaturCover(Number(e.target.value))} className={`${numInput} cursor-pointer`}>
                  {TABUNGAN_SECURITY_GRAMATUR_COVER.map((g) => <option key={g} value={g}>{g} gsm</option>)}
                </select>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {TABUNGAN_SECURITY_MESIN_ISI.map((m) => (
              ...
              <div className="grid grid-cols-2 gap-2 mt-1.5">
                ...
                <select value={gramaturIsi} onChange={(e) => setGramaturIsi(Number(e.target.value))} className={`${numInput} cursor-pointer`}>
                  {TABUNGAN_SECURITY_GRAMATUR_ISI.map((g) => <option key={g} value={g}>{g} gsm</option>)}
                </select>
              </div>
            </div>
            ...
            <div>
              ...
              <select value={finishing} onChange={(e) => setFinishing(e.target.value as TabunganSecurityFinishing)} className={`${numInput} cursor-pointer`}>
                {TABUNGAN_SECURITY_FINISHING.map((f) => <option key={f} value={f}>{f.replace(/,$/, '')}</option>)}
              </select>
            </div>
