...
import {
  ...
  recalculateBukuSoftCover105x148Matrix,
...
export default function BukuSoftCover105x148MatrixView({
  customParams,
  ...
  const matrixData = useMemo(() => {
    return recalculateBukuSoftCover105x148Matrix(
      customParams,
      finishing,
      jilid,
      marginPct,
  ...
  }, [customParams, finishing, jilid, marginPct, negoPct]);
    ...
    <div className="flex flex-col gap-4 pb-8">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-slate-800 text-sm sm:text-base">
                Pricelist Matriks Buku Soft Cover 10,5×14,8 cm
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Matriks harga 22 tier oplah (20 s/d 5.000 pcs) Buku Soft Cover A6 32 Halaman terhitung instan.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {/* Jilid */}
          <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-lg border border-slate-200">
            <span className="text-[11px] font-bold text-slate-500 pl-1.5">Jilid:</span>
            {JILID_OPTIONS.map((j) => (
              <button
                key={j}
                type="button"
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 border border-slate-200">
                    {item.prosesCetak.includes('Print-Isi Print')
                      ? 'Print Inter'
                      : item.prosesCetak.includes('Isi Ryobi')
                      ? 'Ryobi'
                      : 'Oliver'}
                  </span>
                </div>
