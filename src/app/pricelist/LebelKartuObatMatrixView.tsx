...
interface LebelKartuObatMatrixViewProps {
  customParams?: LebelKartuObatMasterParams;
  viewMode?: 'matrix' | 'table';
  setViewMode?: (mode: 'matrix' | 'table') => void;
}

const VARIAN_LIST: LebelKartuObatVarianType[] = ['3,5 x 7 cm', '4 x 6 cm', '5 x 6,7 cm'];

export default function LebelKartuObatMatrixView({
  customParams = DEFAULT_LEBEL_KARTU_OBAT_PARAMS,
  viewMode: propViewMode,
  setViewMode: propSetViewMode,
}: LebelKartuObatMatrixViewProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedVarianFilter, setSelectedVarianFilter] = useState<LebelKartuObatVarianType | 'ALL'>('ALL');
  ...
  const matrixData = useMemo(() => {
    ...
    return LEBEL_KARTU_OBAT_TIERS.map((oplah) => {
  ...
  const flatTableRows = useMemo(() => {
    ...
    varians.forEach((varian) => {
      LEBEL_KARTU_OBAT_TIERS.forEach((oplah) => {
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center gap-3 text-xs">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
          ...
          <select
            ...
            onChange={(e) => setSelectedVarianFilter(e.target.value as LebelKartuObatVarianType | 'ALL')}
      ...
      {viewMode === 'matrix' ? (
                    ...
                    <table className="w-full text-xs text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {matrixData.map((row) => {
                          ...
                          if (!col) return null;
                          return (
                            <tr key={row.oplah} className="hover:bg-cyan-50/30 transition-colors">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden flex flex-col">
                ...
                {flatTableRows.length === 0 ? (
                  ...
                  flatTableRows.map((row, idx) => (
                    <tr key={idx} className="hover:bg-cyan-50/40 transition-colors">
                      <td className="py-2 px-3 font-bold text-slate-800 font-sans">{row.oplah.toLocaleString('id-ID')} rim</td>
          ...
          <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 font-medium">
            Menampilkan {flatTableRows.length} kombinasi tarif Lebel Kartu Obat
          </div>
        </div>
      )}
    </div>
