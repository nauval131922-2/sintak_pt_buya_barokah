...
const DRAFT_KEY = 'sintak_buku_soft_cover_105x148_draft';
...
import {
  ...
  BukuSoftCover105x148SimulatorInput,
  BukuSoftCover105x148SimulatorResult,
  BukuSoftCover105x148FinishingOption,
  BukuSoftCover105x148JilidOption,
  BUKU_SOFT_COVER_105X148_TIERS,
  calculateBukuSoftCover105x148Simulator,
} from '@/lib/buku-soft-cover-105x148-calculator';

export type SavedBukuSoftCover105x148SimulationItem = {
  id: string;
  savedAt: string;
  title: string;
  oplah: number;
...
const JILID_OPTIONS: BukuSoftCover105x148JilidOption[] = [
  'Staples Tengah',
  'Lem Bending',
];
...
export default function BukuSoftCover105x148Simulator({
  customParams,
  onOpenMasterParam,
  activeSimulationId,
  ...
  const [finishing, setFinishing] = useState<BukuSoftCover105x148FinishingOption>(() => draftVal('finishing', 'Tanpa Laminasi'));
  const [jilid, setJilid] = useState<BukuSoftCover105x148JilidOption>(() => draftVal('jilid', 'Staples Tengah'));
  ...
  const result = useMemo<BukuSoftCover105x148SimulatorResult>(() => {
    return calculateBukuSoftCover105x148Simulator(
      {
        oplah,
        finishing,
        jilid,
  ...
  useEffect(() => {
    if (activeSimulationId) {
      try {
        const raw = localStorage.getItem('sintak_saved_buku_soft_cover_105x148_simulations');
        if (raw) {
          const list: SavedBukuSoftCover105x148SimulationItem[] = JSON.parse(raw);
  ...
  const handleSaveSimulation = () => {
    try {
      const existingRaw = localStorage.getItem('sintak_saved_buku_soft_cover_105x148_simulations');
      const list: SavedBukuSoftCover105x148SimulationItem[] = existingRaw ? JSON.parse(existingRaw) : [];

      const title =
        activeSimulationTitle ||
        `Buku Soft Cover 10,5×14,8 • ${oplah.toLocaleString('id-ID')} pcs • ${finishing}`;
      ...
      if (activeSimulationId) {
        ...
        if (targetItem) saveCalculationToDb({ ...targetItem, category: 'Buku Soft Cover 10,5×14,8' });
        ...
        const newItem: SavedBukuSoftCover105x148SimulationItem = {
          id: `bsc_105x148_${Date.now()}`,
          savedAt: new Date().toISOString(),
          title,
          oplah,
          data: result,
        ...
        localStorage.setItem('sintak_saved_buku_soft_cover_105x148_simulations', JSON.stringify(list));
    saveCalculationToDb({ ...newItem, category: 'Buku Soft Cover 10,5×14,8' });
  ...
  const handleSaveAsNew = () => {
    try {
      const existingRaw = localStorage.getItem('sintak_saved_buku_soft_cover_105x148_simulations');
      const list: SavedBukuSoftCover105x148SimulationItem[] = existingRaw ? JSON.parse(existingRaw) : [];
      const title = `Buku Soft Cover 10,5×14,8 • ${oplah.toLocaleString("id-ID")} pcs • ${finishing} • ${jilid}`;
      const newItem: SavedBukuSoftCover105x148SimulationItem = {
        id: `bsc_105x148_${Date.now()}`,
        savedAt: new Date().toISOString(),
        title,
  ...
  const handleCopyWhatsApp = () => {
    ...
    const text = `*PENAWARAN BUKU SOFT COVER 10,5×14,8 CM (A6)*
*PT Buya Barokah*
━━━━━━━━━━━━━━━━━━━━
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-slate-800 text-sm sm:text-base">
                Simulator &amp; Kalkulator Buku Soft Cover 10,5×14,8 cm
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Kalkulasi instan HPP, harga jual, dan batas nego untuk Buku Soft Cover A6 (32 Halaman).
            </p>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  value={BUKU_SOFT_COVER_105X148_TIERS.includes(oplah) ? oplah : 'custom'}
                  onChange={(e) => {
                ...
                >
                  {BUKU_SOFT_COVER_105X148_TIERS.map((t) => (
                    <option key={t} value={t}>
                      {t.toLocaleString('id-ID')} pcs
                    </option>
                  ))}
                  {!BUKU_SOFT_COVER_105X148_TIERS.includes(oplah) && (
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between text-xs text-slate-700">
            ...
            <span className="text-[11px] text-slate-500">
              Cover AC 230 • Isi HVS 70 32 Hal (10,5×14,8 cm)
            </span>
          </div>
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base">Panduan Simulator Buku Soft Cover 10,5×14,8 cm</h3>
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs text-slate-600 leading-relaxed">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <p>
                  Pilih oplah pesanan dari dropdown tier atau ketik angka custom.
                  Sistem otomatis mengarahkan ke alur mesin cetak: Print-Print (≤200), Print-Ryobi (250–600), Print-Oliver (700–1000), atau Oliver-Oliver (≥1500 pcs).
                </p>
              </div>

              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                <h5 className="font-bold text-slate-800">2. Finishing &amp; Jilid</h5>
                <p>
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <p>
                  Gunakan tombol <strong>Salin Penawaran</strong> untuk teks WhatsApp siap kirim ke konsumen,
