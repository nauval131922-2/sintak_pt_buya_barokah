...
const VARIAN_LIST: SyahadahVarianType[] = [
  '1 Muka FC',
  '1 Muka 1 Warna',
  '1 Muka 2 Warna',
  '2 Muka 1 Warna',
...
export default function SyahadahMatrixView({
...
}: SyahadahMatrixViewProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedVarianFilter, setSelectedVarianFilter] = useState<SyahadahVarianType | 'ALL'>('ALL');
  ...
  const matrixData = useMemo(() => {
    ...
    return SYAHADAH_TIERS.map((oplah) => {
    ...
    }).filter(Boolean) as { oplah: number; cols: { varian: SyahadahVarianType; hpp: number; jual: number; nego: number; totalJual: number }[] }[];
  }, [customParams, searchTerm, selectedVarianFilter]);

  // Flat table
  const flatTableRows = useMemo(() => {
    const list: {
      oplah: number; varian: SyahadahVarianType; hpp: number; jual: number; nego: number; totalJual: number; margin: number;
    }[] = [];

    const varians = selectedVarianFilter === 'ALL' ? VARIAN_LIST : [selectedVarianFilter];

    varians.forEach((varian) => {
      SYAHADAH_TIERS.forEach((oplah) => {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm sm:text-base font-bold text-emerald-950 tracking-tight">
              Pricelist Matriks Syahadah
            </h2>
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Tabel perbandingan HPP &amp; harga jual Syahadah Linen/Hammer 21,5×33 cm per oplah &amp; varian (margin 30%, nego 4%, tanpa foil &amp; sisir ON).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center gap-3 text-xs">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
          ...
          <select
            ...
            onChange={(e) => setSelectedVarianFilter(e.target.value as SyahadahVarianType | 'ALL')}
      ...
      {viewMode === 'matrix' ? (
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-gray-800 tracking-tight">Syahadah 21,5×33 cm — Linen/Hammer</h3>
                </div>
                <span className="text-[11px] font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-700">
                  {selectedVarianFilter === 'ALL' ? 'Semua Varian (6)' : `${selectedVarianFilter}`}
                </span>
              </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden flex flex-col">
                ...
                {flatTableRows.length === 0 ? (
                  ...
                  flatTableRows.map((row, idx) => (
                    <tr key={idx} className="hover:bg-emerald-50/40 transition-colors">
                      ...
                      <td className="py-2 px-3 text-slate-500 font-sans">21,5×33</td>
          ...
          <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 font-medium">
            Menampilkan {flatTableRows.length} kombinasi tarif Syahadah
          </div>
        </div>
      )}
    </div>
