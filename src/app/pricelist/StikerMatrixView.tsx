...
export default function StikerMatrixView({
  customParams,
  ...
  const [selectedUkuran, setSelectedUkuran] = useState<string>('ALL');
  ...
  const matrixData = useMemo(() => {
    return recalculateStikerMatrix(
      customParams,
      finishing,
      marginPct,
      negoPct
    );
  }, [customParams, finishing, marginPct, negoPct]);

  const filteredData = useMemo(() => {
    return matrixData.filter((item) => {
      if (selectedUkuran !== 'ALL' && item.ukuran !== selectedUkuran) {
        return false;
      }
  ...
  }, [matrixData, selectedUkuran, searchTerm]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Matriks harga 5 ukuran Stiker Vinyl Glossy across tier oplah terhitung instan.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-lg border border-slate-200">
            <span className="text-[11px] font-bold text-slate-500 pl-1.5">Ukuran:</span>
            <button
              type="button"
              onClick={() => setSelectedUkuran('ALL')}
          ...
          {/* Finishing */}
          <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-lg border border-slate-200">
            <span className="text-[11px] font-bold text-slate-500 pl-1.5">Finishing:</span>
      ...
      {viewMode === 'matrix' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3.5">
          {filteredData.map((item, idx) => (
            <div
              key={`${item.ukuran}-${item.oplah}-${idx}`}
            ...
            <table className="w-full text-xs text-left border-collapse">
              <thead>
                <tr className="bg-slate-100 text-slate-700 border-b border-slate-200 text-[11px]">
                  ...
                  <th className="py-2.5 px-3 font-bold">Ukuran Stiker</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.map((row, idx) => (
                  <tr key={`${row.ukuran}-${row.oplah}-${idx}`} className="hover:bg-slate-50/80 transition-colors">
