...
interface StopmapMatrixViewProps {
...
export default function StopmapMatrixView({
...
}: StopmapMatrixViewProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedUkuranFilter, setSelectedUkuranFilter] = useState<StopmapUkuranType | 'ALL'>('ALL');
  ...
  const matrixData = useMemo(() => {
    ...
    return STOPMAP_TIERS.map((oplah) => {
    ...
    }).filter(Boolean) as { oplah: number; cols: { ukuran: StopmapUkuranType; hpp: number; jual: number; nego: number; totalJual: number }[] }[];
  }, [customParams, searchTerm, selectedUkuranFilter]);

  // Flat table
  const flatTableRows = useMemo(() => {
    const list: {
      oplah: number; ukuran: StopmapUkuranType; hpp: number; jual: number; nego: number; totalJual: number; margin: number;
    }[] = [];

    const ukurans = selectedUkuranFilter === 'ALL' ? UKURAN_LIST : [selectedUkuranFilter];

    ukurans.forEach((ukuran) => {
      STOPMAP_TIERS.forEach((oplah) => {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm sm:text-base font-bold text-emerald-950 tracking-tight">
              Pricelist Matriks Stopmap
            </h2>
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Tabel perbandingan HPP &amp; harga jual Stopmap Soft Cover per oplah &amp; ukuran (margin 30%, nego 4%, laminasi glossy &amp; sisir ON).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center gap-3 text-xs">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
          ...
          <select
            ...
            onChange={(e) => setSelectedUkuranFilter(e.target.value as StopmapUkuranType | 'ALL')}
      ...
      {viewMode === 'matrix' ? (
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-gray-800 tracking-tight">Stopmap</h3>
                </div>
                <span className="text-[11px] font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-700">
                  {selectedUkuranFilter === 'ALL' ? 'Semua Ukuran' : `${selectedUkuranFilter}`}
                </span>
              </div>
                ...
                <div key={ukuran} className="bg-white rounded-xl border border-gray-200 shadow-xs overflow-hidden">
                  <div className="bg-amber-50/70 px-4 py-2 border-b border-amber-100 flex items-center justify-between">
                    <span className="text-[11px] font-bold text-amber-900 tracking-wider uppercase flex items-center gap-1.5">
                      ...
                      Bahan: {ukuran} — {STOPMAP_CONFIG[ukuran].w}×{STOPMAP_CONFIG[ukuran].h} cm · {STOPMAP_CONFIG[ukuran].pcsPerA3} pcs/A3+
                    </span>
                  </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden flex flex-col">
                ...
                {flatTableRows.length === 0 ? (
                  ...
                  flatTableRows.map((row, idx) => (
                    <tr key={idx} className="hover:bg-emerald-50/40 transition-colors">
                      ...
                      <td className="py-2 px-3 text-slate-500 font-sans">Glossy</td>
          ...
          <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 font-medium">
            Menampilkan {flatTableRows.length} kombinasi tarif Stopmap
          </div>
        </div>
      )}
    </div>
