...
interface KartuKoperasiPromiseMatrixViewProps {
  customParams?: KartuKoperasiPromiseMasterParams;
  viewMode?: 'matrix' | 'table';
  setViewMode?: (mode: 'matrix' | 'table') => void;
}

const VARIAN_LIST: KartuKoperasiPromiseVarianType[] = ['10,5 x 16,5', '10,5 x 21,5', '12,7 x 16,3'];

export default function KartuKoperasiPromiseMatrixView({
  customParams = DEFAULT_KARTU_KOPERASI_PROMISE_PARAMS,
  viewMode: propViewMode,
  setViewMode: propSetViewMode,
}: KartuKoperasiPromiseMatrixViewProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedVarianFilter, setSelectedVarianFilter] = useState<KartuKoperasiPromiseVarianType | 'ALL'>('ALL');
  ...
  // Spek default sesuai file Excel tersimpan: Ryobi, 2 Muka, 1 Warna, Tanpa Finishing, margin 30%.
  const calc = (oplah: number, varian: KartuKoperasiPromiseVarianType) =>
    calculateKartuKoperasiPromiseHpp(
      { oplah, varian, mesin: 'Ryobi', muka: '2 Muka', warna: '1 Warna', finishing: 'None,', marginPct: 30 },
      customParams
    );

  const matrixData = useMemo(() => {
    const varians = selectedVarianFilter === 'ALL' ? VARIAN_LIST : [selectedVarianFilter];
    return KARTU_KOPERASI_PROMISE_TIERS.map((oplah) => {
    ...
    }).filter(Boolean) as { oplah: number; cols: { varian: KartuKoperasiPromiseVarianType; hpp: number; jual: number; totalJual: number }[] }[];
  }, [customParams, searchTerm, selectedVarianFilter]);

  const flatTableRows = useMemo(() => {
    const list: {
      oplah: number; varian: KartuKoperasiPromiseVarianType; hpp: number; jual: number; totalJual: number; margin: number;
    }[] = [];

    const varians = selectedVarianFilter === 'ALL' ? VARIAN_LIST : [selectedVarianFilter];

    varians.forEach((varian) => {
      KARTU_KOPERASI_PROMISE_TIERS.forEach((oplah) => {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm sm:text-base font-bold text-emerald-950 tracking-tight">
              Pricelist Matriks Kartu Koperasi Promise
            </h2>
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Tabel perbandingan HPP &amp; harga jual BC 160 2 Muka 1 Warna cetak Ryobi per oplah &amp; varian (margin 30%, pound + sisir + packing).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center gap-3 text-xs">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
          ...
          <select
            ...
            onChange={(e) => setSelectedVarianFilter(e.target.value as KartuKoperasiPromiseVarianType | 'ALL')}
      ...
      {viewMode === 'matrix' ? (
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-gray-800 tracking-tight">Kartu Koperasi Promise — BC 160 2 Muka 1 Warna · Ryobi · Pound + Sisir + Packing</h3>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <div key={varian} className="bg-white rounded-xl border border-gray-200 shadow-xs overflow-hidden">
                  <div className="bg-violet-50/70 px-4 py-2 border-b border-violet-100 flex items-center justify-between">
                    <span className="text-[11px] font-bold text-violet-900 tracking-wider uppercase flex items-center gap-1.5">
                      <Layers size={13} className="text-violet-600" />
                      Varian: {varian} — {KARTU_KOPERASI_PROMISE_CONFIG[varian].kartuW}×{KARTU_KOPERASI_PROMISE_CONFIG[varian].kartuH} cm · {KARTU_KOPERASI_PROMISE_CONFIG[varian].kartuRyobi} kartu/plano · {KARTU_KOPERASI_PROMISE_CONFIG[varian].potongRyobi} potong/plano
                    </span>
                  </div>
                    ...
                    <table className="w-full text-xs text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {matrixData.map((row) => {
                          ...
                          if (!col) return null;
                          return (
                            <tr key={row.oplah} className="hover:bg-violet-50/30 transition-colors">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden flex flex-col">
          ...
          <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 font-medium">
            Menampilkan {flatTableRows.length} kombinasi tarif Kartu Koperasi Promise
          </div>
        </div>
      )}
    </div>
