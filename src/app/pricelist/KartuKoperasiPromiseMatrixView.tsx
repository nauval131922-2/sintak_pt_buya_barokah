...
export default function KartuKoperasiPromiseMatrixView({
  ...
  const matrixData = useMemo(() => {
    ...
    }).filter(Boolean) as { oplah: number; cols: { varian: KartuKoperasiPromiseVarianType; hpp: number; jual: number; nego: number; totalJual: number }[] }[];
  }, [customParams, searchTerm, selectedVarianFilter]);

  const flatTableRows = useMemo(() => {
    const list: {
      oplah: number; varian: KartuKoperasiPromiseVarianType; hpp: number; jual: number; nego: number; totalJual: number; margin: number;
    }[] = [];
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Tabel perbandingan HPP &amp; harga jual Kartu Koperasi Promise BC 160 2 Muka 1 Warna per oplah &amp; varian (margin 30%, nego 4%, pound + sisir + packing).
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-gray-800 tracking-tight">Kartu Koperasi Promise — BC 160 2 Muka 1 Warna · Pound + Sisir + Packing</h3>
                </div>
                ...
                <div key={varian} className="bg-white rounded-xl border border-gray-200 shadow-xs overflow-hidden">
                  <div className="bg-violet-50/70 px-4 py-2 border-b border-violet-100 flex items-center justify-between">
                    <span className="text-[11px] font-bold text-violet-900 tracking-wider uppercase flex items-center gap-1.5">
                      ...
                      Varian: {varian} — {KARTU_KOPERASI_PROMISE_CONFIG[varian].w}×{KARTU_KOPERASI_PROMISE_CONFIG[varian].h} cm · {KARTU_KOPERASI_PROMISE_CONFIG[varian].kartuPerPlano} kartu/plano · {KARTU_KOPERASI_PROMISE_CONFIG[varian].potongPerPlano} potong/plano · {KARTU_KOPERASI_PROMISE_CONFIG[varian].platCount} plat
                    </span>
                  </div>
