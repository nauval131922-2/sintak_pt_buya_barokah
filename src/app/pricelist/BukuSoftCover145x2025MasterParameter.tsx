...
export default function BukuSoftCover145x2025MasterParameter({
  ...
  const isFieldModified = (key: keyof BukuSoftCover145x2025MasterParams) => {
  ...
  const handleResetField = (key: keyof BukuSoftCover145x2025MasterParams) => {
    setCustomParams((prev) => ({
      ...prev,
      [key]: DEFAULT_BUKU_SOFT_COVER_145X2025_PARAMS[key],
    }));
  };
  ...
  const handleChange = (
    key: keyof BukuSoftCover145x2025MasterParams,
    value: string,
    isFloat = false
  ) => {
      ...
      >
        <div className="flex flex-col min-w-0 pr-2">
          ...
          <span className="text-[10px] text-slate-400">
            Def:{' '}
            {isCurrency
              ? `Rp ${DEFAULT_BUKU_SOFT_COVER_145X2025_PARAMS[key].toLocaleString('id-ID')}`
              : `${DEFAULT_BUKU_SOFT_COVER_145X2025_PARAMS[key]}${key.includes('Pct') ? '%' : ''}`}
          </span>
        </div>
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-slate-800 text-sm sm:text-base">
                Master Parameter Buku Soft Cover 14,5×20,25 cm
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Tarif bahan, mesin bertingkat (Print, Ryobi, Oliver), finishing, dan margin HPP terverifikasi.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs flex flex-col gap-3">
          ...
          <p className="text-[10px] text-slate-500">
            Print Inter A3+ untuk oplah ≤600 pcs. Oliver offset 4W untuk oplah ≥650 pcs (4 plat).
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs flex flex-col gap-3">
          ...
          <p className="text-[10px] text-slate-500">
            Print Buya (≤200 pcs), Ryobi (250–900 pcs, 16 plat), Oliver (≥1000 pcs, 8 plat).
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {fieldRow('jasaFinishingStandar', 'Finishing Susun/Staples (Rp/pcs)', true, true)}
            {fieldRow('tarifStaplesPcs', 'Isi Staples (Rp/pcs)', true)}
            {fieldRow('tarifSisirPerPcs', 'Sisir (Rp/pcs)', true)}
            {fieldRow('tarifKardusBox', 'Kardus Box (Rp/box)', true)}
            {fieldRow('tarifLakbanRoll', 'Lakban Roll (Rp/roll)', true)}
          </div>
          <p className="text-[10px] text-slate-500">
            Susun Lipat + Staples + Sisir + Packing Kardus (kapasitas 200 pcs/box).
          </p>
        </div>

        {/* Card 4: Finishing Tambahan & Margin */}
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs flex flex-col gap-3">
          <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base">Panduan Parameter Buku Soft Cover 14,5×20,25 cm</h3>
                  <p className="text-xs text-emerald-200">Referensi: Pricelist Buku Soft Cover - 14,5 x 20,25 cm.xlsx</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs text-slate-600 leading-relaxed">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                <h5 className="font-bold text-slate-800 flex items-center gap-1.5">
                  ...
                  Spesifikasi Standar Buku
                </h5>
                <p>
                  Ukuran tertutup 14,5 × 20,25 cm (terbuka 29 × 20,25 cm + margin potong = 30 × 21,25 cm = 637,5 cm²).
                  Cover Art Carton 230 gsm 1 Muka Full Colour. Isi HVS 70 gsm 1 Warna Bolak-Balik 32 Halaman (8 Lembar).
                </p>
              </div>
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                <h5 className="font-bold text-slate-800 flex items-center gap-1.5">
                  ...
                  Alur Proses Produksi Bertingkat
                </h5>
                <ul className="list-disc list-inside space-y-1 pl-1">
                  <li><strong>Oplah 20–200 pcs</strong>: Cover Print Inter A3+ + Isi Print Buya A4/A3+</li>
                  <li><strong>Oplah 250–600 pcs</strong>: Cover Print Inter A3+ + Isi Ryobi Offset 1W (16 Plat)</li>
                  <li><strong>Oplah 650–900 pcs</strong>: Cover Oliver Offset 4W (4 Plat) + Isi Ryobi Offset 1W (16 Plat)</li>
                  <li><strong>Oplah 1000–3000 pcs</strong>: Cover Oliver Offset 4W (4 Plat) + Isi Oliver Offset 1W (8 Plat)</li>
                </ul>
              </div>
