...
export default function PackagingMatrixView({
  ...
  const [selectedFinishing, setSelectedFinishing] = useState<PackagingFinishing>('Tanpa Laminasi');
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [priceType, setPriceType] = useState<'harga' | 'nego' | 'hpp'>('harga');

  // Generate matrix data across all 12 variants and all oplahs
  const matrixData = useMemo(() => {
    const variantKeys = Object.keys(PACKAGING_VARIANTS);

    return variantKeys.map((key) => {
      const spec = PACKAGING_VARIANTS[key];
      const rows = PACKAGING_OPLAH_OPTIONS.map((oplah) => {
  ...
  const filteredMatrix = useMemo(() => {
    return matrixData.filter((item) => {
      if (filterUkuran !== 'ALL' && item.spec.ukuran !== filterUkuran) return false;
      if (filterBahan !== 'ALL' && item.spec.bahan !== filterBahan) return false;
      if (searchTerm.trim()) {
        const q = searchTerm.toLowerCase();
        const matchName = item.spec.namaProduk.toLowerCase().includes(q);
        const matchUk = item.spec.ukuran.toLowerCase().includes(q);
        const matchBahan = item.spec.bahan.toLowerCase().includes(q);
        if (!matchName && !matchUk && !matchBahan) return false;
      }
      return true;
    });
  }, [matrixData, filterUkuran, filterBahan, searchTerm]);
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-lg font-bold text-slate-900 tracking-tight">
                Matrix Pricelist Packaging Box Dus
              </h2>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Tabel perbandingan harga jual, nego, dan HPP packaging box dus untuk seluruh ukuran dan oplah
            </p>
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-2xs flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 text-xs">
            ...
            >
              <option value="ALL">Semua Ukuran (4)</option>
              {PACKAGING_UKURAN_OPTIONS.map((uk) => (
          ...
          {/* Filter Bahan */}
          <div className="flex items-center gap-1.5 text-xs">
            <span className="font-bold text-slate-600">Bahan:</span>
            <select
              value={filterBahan}
              onChange={(e) => setFilterBahan(e.target.value)}
              className="py-1 px-2.5 rounded-lg border border-slate-300 text-xs font-semibold text-slate-800 bg-slate-50 focus:bg-white focus:outline-none focus:border-emerald-500"
            >
              <option value="ALL">Semua Bahan (3)</option>
              {PACKAGING_BAHAN_OPTIONS.map((bh) => (
          ...
          <div className="flex items-center gap-1.5 text-xs">
            ...
            <select
              ...
              onChange={(e) => setSelectedFinishing(e.target.value as PackagingFinishing)}
              className="py-1 px-2.5 rounded-lg border border-slate-300 text-xs font-semibold text-slate-800 bg-slate-50 focus:bg-white focus:outline-none focus:border-emerald-500"
            >
              {PACKAGING_FINISHING_OPTIONS.map((fin) => (
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-slate-700 font-bold uppercase tracking-wider text-[10px]">
                ...
                {PACKAGING_OPLAH_OPTIONS.map((oplah) => (
                  <th
                    key={oplah}
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
              {filteredMatrix.length === 0 ? (
                <tr>
                  <td
                    colSpan={4 + PACKAGING_OPLAH_OPTIONS.length}
                    className="py-8 text-center text-slate-600"
                  >
                    Tidak ada varian packaging box yang cocok dengan filter.
                  </td>
                </tr>
              ) : (
