...
export default function BukuTabunganNsSimulator({
  ...
  const [varian, setVarian] = useState<BukuTabunganNsVarianType>('24 Hal');
  ...
  const handleSaveSimulation = () => {
    ...
    try {
      ...
      console.error('Failed to save buku tabungan ns simulation:', e);
    }
    setSimulationTitle('');
  ...
  const handleUpdateSavedSimulation = () => {
    ...
    try {
      ...
      console.error('Failed to update buku tabungan ns simulation:', e);
    }
  ...
  const handleCopyQuote = () => {
    ...
    const cfg = BUKU_TABUNGAN_NS_CONFIG[varian];
    const text =
      `*PENAWARAN BUKU TABUNGAN NON SECURITY*\n` +
      `*PT Buya Barokah*\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `• *Produk*: Buku Tabungan NS ${varian} 9×14,5 cm\n` +
      `• *Spesifikasi*: ${cfg.description}\n` +
      `• *Ukuran*: 9×14,5 cm (tertutup) · Cover AC 260 gsm 1 Muka FC + Laminasi Glossy · ${cfg.jumlahLbr} lbr isi\n` +
      `• *Bahan*: Cover Art Carton 260 gsm, Isi HVS 70 gsm 1 Warna Bolak-Balik\n` +
      ...
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `_Harga belum termasuk PPN. Buku Tabungan Non Security 9×14,5 cm, AC 260 + HVS 70, cetak Print Inter & Ryobi, finishing jahit + pound._`;

    navigator.clipboard.writeText(text);
    setCopiedQuote(true);
    toast.success('Penawaran harga Buku Tabungan NS berhasil disalin ke WhatsApp clipboard!');
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Hitung HPP, harga penawaran, dan estimasi profit Buku Tabungan 9×14,5 cm Non Security 24/32/48 Hal, cover AC 260 FC laminasi + isi HVS 70 1W BB.
            </p>
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-300 rounded-xl p-3 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-in fade-in duration-200">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={() => {
                if (setCustomParams) setCustomParams(DEFAULT_BUKU_TABUNGAN_NS_PARAMS);
                toast.success('Parameter Buku Tabungan NS dikembalikan ke tarif Master Standar!');
              }}
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                Varian Jumlah Halaman (9×14,5 cm)
              </label>
              ...
              <p className="text-[11px] text-slate-500 mt-1.5 italic">{BUKU_TABUNGAN_NS_CONFIG[varian].description}</p>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  value={BUKU_TABUNGAN_NS_TIERS.includes(oplah) ? oplah : 'custom'}
                  onChange={(e) => {
                ...
                >
                  {BUKU_TABUNGAN_NS_TIERS.map((t) => (
                    <option key={t} value={t}>{t.toLocaleString('id-ID')} pcs</option>
                  ))}
                  {!BUKU_TABUNGAN_NS_TIERS.includes(oplah) && <option value="custom">{oplah.toLocaleString('id-ID')} pcs (custom)</option>}
                </select>
                <input
                  type="number"
                  min={1}
              ...
              <p className="text-[10px] text-slate-500 mt-1">Cover {BUKU_TABUNGAN_NS_CONFIG[varian].coverPcsPerA3} pcs/A3+ · Isi {BUKU_TABUNGAN_NS_CONFIG[varian].leavesPerA3} lbr/A3+ · Cover {result.kebutuhanCoverA3} lbr + Isi {result.kebutuhanIsiA3} lbr A3+</p>
            </div>
            ...
            <div className="px-4 py-3 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                  Rincian Estimasi Komponen Biaya Buku Tabungan
                </h4>
              </div>
              <span className="text-[11px] font-bold text-slate-500">
                {oplah.toLocaleString('id-ID')} pcs · {varian} · {BUKU_TABUNGAN_NS_CONFIG[varian].jumlahLbr} lbr
              </span>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-emerald-200/90 mt-0.5">
                    Alur perhitungan berbasis oplah pcs, 9×14,5 cm Non Security 24/32/48 Hal, AC 260 + HVS 70, jahit + pound
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-xs text-slate-700 leading-relaxed">
              <div className="space-y-3">
                <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                  ...
                  Langkah Menggunakan Simulator Buku Tabungan
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  {[
                    ['1. Varian', 'Pilih 24 Hal (6 lbr), 32 Hal (8 lbr), atau 48 Hal (12 lbr) — beda isi HVS 70 & cetak.'],
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2.5">
                <h5 className="font-bold text-slate-900 text-xs uppercase tracking-wider flex items-center gap-2">
                  ...
                  Struktur Biaya Produksi Buku Tabungan
                </h5>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px]">
                  <div className="p-2.5 bg-white rounded border border-emerald-100 space-y-1">
                    ...
                    <p className="text-slate-600 leading-snug">
                      Cover AC 260 gsm Rp 16.400/kg +5% 4 pcs/A3+ insheet 15, isi HVS 70 gsm Rp 15.700/kg +5% 6 lbr/A3+ insheet 30, cetak cover Print Inter Rp 3.500/A3+ 1 Muka FC, cetak isi Ryobi Rp 1.500/A3+ 1W BB (≤500) → Oliver ＞500 0.6× + plat 180k, desain cover Rp 15.000 + isi Rp 1.500/lbr.
                    </p>
                  </div>
                  <div className="p-2.5 bg-white rounded border border-blue-100 space-y-1">
                    <span className="font-bold text-blue-900 block">Laminasi &amp; Finishing:</span>
                    <p className="text-slate-600 leading-snug">
                      Laminasi glossy Rp 0,35/cm² min Rp 50.000 luas (9×2+1)*(14.5+1), susun lipat Rp 125/pcs, jahit Rp 500/pcs min 250k, pound pisau Rp 52.377 + jasa Rp 300/pcs, plastik sring Rp 150/pcs, packing kardus 8.500 + lakban 8.000 per order. Margin 30% nego 5%.
                    </p>
                  </div>
                </div>
              </div>
