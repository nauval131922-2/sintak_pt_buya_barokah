...
import {
  ...
  BukuHardCover145x2025FinishingOption,
  BUKU_HARD_COVER_145X2025_TIERS,
  calculateBukuHardCover145x2025Simulator,
} from '@/lib/buku-hard-cover-145x2025-calculator';

export type SavedBukuHardCover145x2025SimulationItem = {
  id: string;
  savedAt: string;
  title: string;
  oplah: number;
  data: BukuHardCover145x2025SimulatorResult;
  paramsSnapshot?: any;
};

const FINISHING_OPTIONS: BukuHardCover145x2025FinishingOption[] = [
  'Tanpa Laminasi',
  'Laminasi Glossy',
  'Laminasi Doff',
];
...
export default function BukuHardCover145x2025Simulator({
  customParams,
  onOpenMasterParam,
  activeSimulationId,
  ...
  const result = useMemo<BukuHardCover145x2025SimulatorResult>(() => {
    return calculateBukuHardCover145x2025Simulator(
      {
        oplah,
        finishing,
        opsiFoil,
  ...
  useEffect(() => {
    if (activeSimulationId) {
      try {
        const raw = localStorage.getItem('sintak_saved_buku_hard_cover_145x2025_simulations');
        if (raw) {
          const list: SavedBukuHardCover145x2025SimulationItem[] = JSON.parse(raw);
  ...
  const handleSaveSimulation = () => {
    try {
      const existingRaw = localStorage.getItem('sintak_saved_buku_hard_cover_145x2025_simulations');
      const list: SavedBukuHardCover145x2025SimulationItem[] = existingRaw ? JSON.parse(existingRaw) : [];

      const title =
        activeSimulationTitle ||
        `Buku Hard Cover 14,5×20,25 • ${oplah.toLocaleString('id-ID')} pcs • ${finishing}${opsiFoil ? ' + Foil' : ''}`;
      ...
      if (activeSimulationId) {
        ...
        localStorage.setItem('sintak_saved_buku_hard_cover_145x2025_simulations', JSON.stringify(list));
  ...
  const handleSaveAsNew = () => {
    try {
      const existingRaw = localStorage.getItem('sintak_saved_buku_hard_cover_145x2025_simulations');
      const list: SavedBukuHardCover145x2025SimulationItem[] = existingRaw ? JSON.parse(existingRaw) : [];
      const title = `Buku Hard Cover 14,5×20,25 • ${oplah.toLocaleString("id-ID")} pcs • ${finishing}${opsiFoil ? " + Foil" : ""}`;
      const newItem: SavedBukuHardCover145x2025SimulationItem = {
        id: `bhc_145x2025_${Date.now()}`,
        savedAt: new Date().toISOString(),
        title,
  ...
  const handleCopyWhatsApp = () => {
    ...
    const text = `*PENAWARAN BUKU HARD COVER 14,5×20,25 CM*
*PT Buya Barokah*
━━━━━━━━━━━━━━━━━━━━
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-black text-slate-800 text-sm sm:text-base">
                Simulator &amp; Kalkulator Buku Hard Cover 14,5×20,25 cm
              </h3>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Kalkulasi instan HPP, harga jual, dan batas nego untuk Buku Hard Cover 100 Halaman (14,5×20,25 cm).
            </p>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <select
                  value={BUKU_HARD_COVER_145X2025_TIERS.includes(oplah) ? oplah : 'custom'}
                  onChange={(e) => {
                ...
                >
                  {BUKU_HARD_COVER_145X2025_TIERS.map((t) => (
                    <option key={t} value={t}>
                      {t.toLocaleString('id-ID')} pcs
                    </option>
                  ))}
                  {!BUKU_HARD_COVER_145X2025_TIERS.includes(oplah) && (
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between text-xs text-slate-700">
            ...
            <span className="text-[11px] text-slate-500">
              Hard Cover Art Paper 150 + Board • Isi 100 Hal HVS 70 (14,5×20,25 cm)
            </span>
          </div>
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base">Panduan Simulator Buku Hard Cover 14,5×20,25 cm</h3>
