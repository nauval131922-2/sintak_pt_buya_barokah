...
const UKURAN_OPTIONS: StopmapUkuranType[] = ['A4 (22 x 31 cm)', 'FOLIO (24 x 35 cm)'];
const LAMINASI_OPTIONS: StopmapLaminasiType[] = ['Glossy', 'Doff'];
...
export default function StopmapSimulator({
  ...
  const [ukuran, setUkuran] = useState<StopmapUkuranType>('A4 (22 x 31 cm)');
  ...
  useEffect(() => {
    try {
      ...
      console.error('Failed to load saved stopmap simulations:', e);
    }
  }, [activeSimulationId]);
  ...
  const result = useMemo(
    () =>
      calculateStopmapHpp(
        { oplah, ukuran, laminasi, marginPct, negoDiskonPct },
        customParams
      ),
    [oplah, ukuran, laminasi, marginPct, negoDiskonPct, customParams]
  );

  const defaultTitle = () =>
    `Stopmap ${ukuran} ${laminasi} (${oplah} pcs)`;
  ...
  const handleCopyQuote = () => {
    ...
    const text =
      ...
      `*PT Buya Barokah*\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `• *Produk*: Stopmap ${ukuran}\n` +
      `• *Spesifikasi*: ${cfg.description}\n` +
      `• *Kuantitas*: ${oplah} pcs\n` +
      `• *Laminasi*: ${laminasi}${laminasi === 'Doff' ? ' (+Rp 200/pcs)' : ''}\n` +
      ...
      `━━━━━━━━━━━━━━━━━━━━\n` +
      `_Harga belum termasuk PPN. Soft Cover Art Carton 230 gsm 1 muka full colour._`;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Hitung HPP, harga penawaran, dan estimasi profit Stopmap A4 22×31 & Folio 24×35 cm (AC 230 gsm, 1 muka full colour).
            </p>
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-300 rounded-xl p-3 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-in fade-in duration-200">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              type="button"
              onClick={() => {
                if (setCustomParams) setCustomParams(DEFAULT_STOPMAP_PARAMS);
                toast.success('Parameter Stopmap dikembalikan ke tarif Master Standar!');
              }}
        ...
        <div className="lg:col-span-5 space-y-5">
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col gap-4">
            <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
              ...
              <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Input Spesifikasi Stopmap</h3>
            </div>

            {/* Ukuran */}
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
                Ukuran Stopmap (tertutup)
              </label>
              <div className="grid grid-cols-2 gap-2">
                {UKURAN_OPTIONS.map((u) => (
                  ...
                  >
                    <Folder size={14} className={ukuran === u ? 'text-white' : 'text-slate-500'} />
                    <span>{u}</span>
                  </button>
                ))}
              </div>
              <p className="text-[11px] text-slate-500 mt-1.5 italic">{STOPMAP_CONFIG[ukuran].description}</p>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {LAMINASI_OPTIONS.map((l) => (
                  ...
                  >
                    {l === 'Glossy' ? 'Glossy' : 'Doff (+Rp 200/pcs)'}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  {!STOPMAP_TIERS.includes(oplah) && <option value="custom">{oplah.toLocaleString('id-ID')} pcs (custom)</option>}
                </select>
                <input
                  type="number"
                  min={1}
            ...
            <div className="px-4 py-3 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                  Rincian Estimasi Komponen Biaya Stopmap
                </h4>
              </div>
              <span className="text-[11px] font-bold text-slate-500">
                {oplah.toLocaleString('id-ID')} pcs · {ukuran} · {laminasi}
              </span>
            </div>
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold tracking-tight">Panduan Simulator Stopmap</h3>
                  <p className="text-xs text-emerald-200/90 mt-0.5">
                    Alur perhitungan berbasis oplah pcs, AC 230 gsm 1 muka, laminasi & finishing Sisir+Lipat+Kupingan
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-xs text-slate-700 leading-relaxed">
              <div className="space-y-3">
                <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                  ...
                  Langkah Menggunakan Simulator Stopmap
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  {[
                    ['1. Ukuran & Laminasi', 'Pilih A4 22×31 atau Folio 24×35 cm (tertutup) dan pilih Glossy atau Doff (+Rp 200/pcs).'],
                    ['2. Oplah', 'Tentukan oplah 10–3000 pcs via dropdown tier atau input custom.'],
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2.5">
                <h5 className="font-bold text-slate-900 text-xs uppercase tracking-wider flex items-center gap-2">
                  ...
                  Struktur Biaya Produksi Stopmap
                </h5>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px]">
                  <div className="p-2.5 bg-white rounded border border-emerald-100 space-y-1">
                    ...
                    <p className="text-slate-600 leading-snug">
                      AC 230 gsm Rp 16.400/kg + up 5%, 1 pcs/A3+, 1 muka 4 warna @ Rp 2.500/A3+ Print Inter, insheet 5 lbr, desain Rp 10.000/order.
                    </p>
                  </div>
                  <div className="p-2.5 bg-white rounded border border-blue-100 space-y-1">
                    ...
                    <p className="text-slate-600 leading-snug">
                      Laminasi glossy Rp 0,35/cm² (min 50k) + Doff +Rp 200/pcs, sisir+lipat+kupingan per pcs, packing kardus+lakban per order.
                    </p>
                  </div>
                </div>
              </div>
