...
const MESIN_FILTER: TabunganNsMesinIsi[] = ['Otomatis', 'Print Buya', 'Print Inter', 'Ryobi', 'Oliver', 'SM'];
...
export default function BukuTabunganNsMatrixView({
  ...
  const params: BukuTabunganNsMasterParams = { ...DEFAULT_BUKU_TABUNGAN_NS_PARAMS, ...(customParams || {}) };
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedMesin, setSelectedMesin] = useState<TabunganNsMesinIsi>('Otomatis');
  ...
  const matrixData = useMemo(() => {
    return BUKU_TABUNGAN_NS_TIERS.map((pcs) => {
    ...
    <div className="space-y-4">
      <div className="bg-emerald-50/70 border border-emerald-200/80 rounded-2xl p-4 sm:p-5 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm sm:text-base font-bold text-emerald-950 tracking-tight">Pricelist Matriks Buku Tabungan NS</h2>
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center gap-3 text-xs">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
          ...
          <select value={selectedMesin} onChange={(e) => setSelectedMesin(e.target.value as TabunganNsMesinIsi)}
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between border-b border-gray-200 pb-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-gray-800 tracking-tight">Buku Tabungan NS 9 × 14,5 cm — Isi {selectedMesin === 'Otomatis' ? 'Otomatis (Buya/Ryobi)' : selectedMesin}</h3>
            </div>
