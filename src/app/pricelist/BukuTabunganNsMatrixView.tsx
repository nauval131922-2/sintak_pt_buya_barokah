...
interface BukuTabunganNsMatrixViewProps {
  customParams?: BukuTabunganNsMasterParams;
  viewMode?: 'matrix' | 'table';
  setViewMode?: (mode: 'matrix' | 'table') => void;
}

const VARIAN_LIST: BukuTabunganNsVarianType[] = ['24 Hal', '32 Hal', '48 Hal'];

export default function BukuTabunganNsMatrixView({
  customParams = DEFAULT_BUKU_TABUNGAN_NS_PARAMS,
  viewMode: propViewMode,
  setViewMode: propSetViewMode,
}: BukuTabunganNsMatrixViewProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedVarianFilter, setSelectedVarianFilter] = useState<BukuTabunganNsVarianType | 'ALL'>('ALL');
  ...
  const calc = (oplah: number, varian: BukuTabunganNsVarianType) =>
    calculateBukuTabunganNsHpp(
      { oplah, varian, marginPct: 30, negoDiskonPct: 5 },
      customParams
    );

  const matrixData = useMemo(() => {
    const varians = selectedVarianFilter === 'ALL' ? VARIAN_LIST : [selectedVarianFilter];
    return BUKU_TABUNGAN_NS_TIERS.map((oplah) => {
    ...
    }).filter(Boolean) as { oplah: number; cols: { varian: BukuTabunganNsVarianType; hpp: number; jual: number; nego: number; totalJual: number }[] }[];
  }, [customParams, searchTerm, selectedVarianFilter]);

  const flatTableRows = useMemo(() => {
    const list: {
      oplah: number; varian: BukuTabunganNsVarianType; hpp: number; jual: number; nego: number; totalJual: number; margin: number;
    }[] = [];

    const varians = selectedVarianFilter === 'ALL' ? VARIAN_LIST : [selectedVarianFilter];

    varians.forEach((varian) => {
      BUKU_TABUNGAN_NS_TIERS.forEach((oplah) => {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm sm:text-base font-bold text-emerald-950 tracking-tight">
              Pricelist Matriks Buku Tabungan Non Security
            </h2>
            <p className="text-[11.5px] text-emerald-800/80 mt-0.5">
              Tabel perbandingan HPP &amp; harga jual Buku Tabungan 9×14,5 cm NS 24/32/48 Hal per oplah &amp; varian (margin 30%, nego 5%, AC 260 + HVS 70, jahit+pound).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center gap-3 text-xs">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto shrink-0">
          ...
          <select
            ...
            onChange={(e) => setSelectedVarianFilter(e.target.value as BukuTabunganNsVarianType | 'ALL')}
      ...
      {viewMode === 'matrix' ? (
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between border-b border-gray-200 pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-gray-800 tracking-tight">Buku Tabungan 9×14,5 cm — Non Security · AC 260 + HVS 70</h3>
                </div>
              ...
              <div ref={gridRef} className="grid gap-4" style={{ gridTemplateColumns: `repeat(${varianCols.length===1 ? 1 : Math.min(varianCols.length, autoCols)}, minmax(0, 1fr))` }}>
                ...
                <div key={varian} className="bg-white rounded-xl border border-gray-200 shadow-xs overflow-hidden">
                  <div className="bg-teal-50/70 px-4 py-2 border-b border-teal-100 flex items-center justify-between">
                    <span className="text-[11px] font-bold text-teal-900 tracking-wider uppercase flex items-center gap-1.5">
                      <Layers size={13} className="text-teal-600" />
                      Varian: {varian} — {BUKU_TABUNGAN_NS_CONFIG[varian].jumlahHalaman} Hal ({BUKU_TABUNGAN_NS_CONFIG[varian].jumlahLbr} lbr) · {BUKU_TABUNGAN_NS_CONFIG[varian].w}×{BUKU_TABUNGAN_NS_CONFIG[varian].h} cm · {BUKU_TABUNGAN_NS_CONFIG[varian].coverPcsPerA3} cov/A3+ · {BUKU_TABUNGAN_NS_CONFIG[varian].leavesPerA3} isi/A3+
                    </span>
                  </div>
                    ...
                    <table className="w-full text-xs text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {matrixData.map((row) => {
                          ...
                          if (!col) return null;
                          return (
                            <tr key={row.oplah} className="hover:bg-teal-50/30 transition-colors">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden flex flex-col">
          ...
          <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 font-medium">
            Menampilkan {flatTableRows.length} kombinasi tarif Buku Tabungan NS
          </div>
        </div>
      )}
    </div>
