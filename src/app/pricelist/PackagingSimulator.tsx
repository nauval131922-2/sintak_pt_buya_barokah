...
import {
  ...
  PackagingBahan,
  PackagingFinishing,
  PackagingMasterParams,
  DEFAULT_PACKAGING_PARAMS,
  calculatePackaging,
  PACKAGING_UKURAN_OPTIONS,
  PACKAGING_BAHAN_OPTIONS,
  PACKAGING_FINISHING_OPTIONS,
  PACKAGING_OPLAH_OPTIONS,
...
export default function PackagingSimulator({
  ...
  }, [ukuran, bahan, oplah, finishing, marginPct, negoDiskonPct, customParams]);

  // Update simulation title
  useEffect(() => {
    if (setActiveSimulationTitle) {
      setActiveSimulationTitle(
        `Packaging Box Dus ${ukuran} · ${bahan} · ${oplah.toLocaleString('id-ID')} pcs`
      );
    }
  }, [ukuran, bahan, oplah, setActiveSimulationTitle]);
  ...
  const handleReset = () => {
    setUkuran('15 x 15 x 8 cm');
    setBahan('Duplex 350 gsm');
    setOplah(1000);
    setIsCustomOplah(false);
    setCustomOplahInput('');
  ...
  const handleSaveSimulation = () => {
    try {
      const existingStr = localStorage.getItem('sintak_saved_packaging_simulations');
      const list: SavedPackagingSimulationItem[] = existingStr ? JSON.parse(existingStr) : [];
      const title =
        simName.trim() ||
        `Packaging ${ukuran} (${bahan}) - ${oplah.toLocaleString('id-ID')} pcs`;

      const newItem: SavedPackagingSimulationItem = {
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-lg font-bold text-slate-900 tracking-tight">
                Simulator Biaya & Harga Jual Packaging Box Dus
              </h2>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Kalkulator instan HPP, cetak offset (Oliver/SM), pond die cut, dan penawaran box packaging
            </p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-2xs flex flex-col gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {PACKAGING_UKURAN_OPTIONS.map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    onClick={() => setUkuran(opt)}
                    className={`py-2 px-2.5 rounded-xl text-xs font-bold border text-left transition-all cursor-pointer flex flex-col ${
                      ukuran === opt
                  ...
                  >
                    <span>{opt}</span>
                    <span className={`text-[10px] font-normal ${ukuran === opt ? 'text-emerald-200' : 'text-slate-600'}`}>
                      {opt === '15 x 15 x 8 cm'
                        ? 'Dus Snack/Nasi'
                        : opt === '20 x 20 x 7 cm'
                        ? 'Dus Nasi Standar'
                        : opt === '15,2 x 10,2 x 4,5 cm'
                        ? 'Dus Kue Mini'
                        : 'Dus Nasi Sedang'}
                    </span>
                  </button>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {PACKAGING_BAHAN_OPTIONS.map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    onClick={() => setBahan(opt)}
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 mb-1.5 block">
                3. Finishing Tambahan (Opsional)
              </label>
              <div className="grid grid-cols-2 gap-2">
                {PACKAGING_FINISHING_OPTIONS.map((opt) => (
                  <button
                    key={opt}
                    type="button"
            ...
            {/* 4. Pilihan Kuantitas (Oplah) */}
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs font-bold text-slate-700">
                  4. Kuantitas Pesanan (Pcs)
                </label>
              ...
              <div className="grid grid-cols-4 gap-1.5 mb-2.5">
                {PACKAGING_OPLAH_OPTIONS.map((optVal) => (
                  <button
                    key={optVal}
                    type="button"
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-2xs flex flex-col">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                  Rincian Breakdown HPP Packaging Box Dus
                </h3>
              </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
              ...
              <div className="p-2 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <span className="text-xs font-bold text-slate-800">
                  {result.variantSpec.planoYield} box / plano
                </span>
              </div>
              <div className="p-2 bg-slate-50 rounded-xl border border-slate-200">
                <span className="text-[10px] text-slate-500 block">Ukuran Terbuka</span>
                <span className="text-xs font-bold text-slate-800">
                  {result.variantSpec.ukuranTerbuka}
                </span>
              </div>
              <div className="p-2 bg-slate-50 rounded-xl border border-slate-200">
          ...
          >
            <div className="px-6 py-4 bg-emerald-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-base text-white">Panduan Simulator Packaging Box Dus</h3>
