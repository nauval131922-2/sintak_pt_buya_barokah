...
export default function PelunasanPiutangClient() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      ...
      return saved ? JSON.parse(saved) : {
        id: 100, tgl: 120, faktur: 220, kd_pelanggan: 300, kredit: 180, username: 120, recid: 80
      };
    }
    return {};
  });
  ...
  const handleFetch = async () => {
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-pelunasan-piutang?start=${chunk.start}&end=${chunk.end}&metaStart=${startStr}&metaEnd=${endStr}&silent=true`);
        if (res.ok) {
    ...
    try {
      ...
      if (successCount > 0) {
        persistScraperPeriod({ stateKey: 'ppState', periodKey: 'PelunasanPiutangClient_scrapedPeriod' }, startDate, endDate);
        persistScraperPeriodFull('last_scrape_pelunasan_piutang_period', startDate, endDate);
        setRefreshKey(prev => prev + 1);
        localStorage.setItem('sintak_data_updated', Date.now().toString());
        fetch('/api/activity-log', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action_type: 'SCRAPE', table_name: 'pelunasan_piutang', message: `Scrape pelunasan piutang berhasil: ${totalScraped} baris (${startStr} - ${endStr}).`, raw_data: JSON.stringify({ total: totalScraped, start: startStr, end: endStr }) }),
        }).catch(() => {});
        setDialog({ isOpen: true, type: 'success', title: 'Berhasil', message: `Berhasil menarik ${totalScraped} data Pelunasan Piutang Penjualan.` });
      }
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      header: 'Tanggal',
      size: 120,
      cell: ({ getValue, row }: any) => <span className={`font-bold tabular-nums ${row.getIsSelected() ? 'text-green-700' : 'text-gray-700'}`}>{formatISODateStr(getValue() as string)}</span>
    },
    {
      accessorKey: 'faktur',
      header: 'Faktur PP',
    ...
    },
    {
      ...
      size: 160,
      meta: { align: 'right' },
      cell: ({ getValue, row }: any) => <span className={`font-bold tabular-nums ${row.getIsSelected() ? 'text-green-800' : 'text-gray-700'}`}>{Number(getValue() || 0).toLocaleString('id-ID')}</span>
    },
    {
      accessorKey: 'kredit',
      ...
      <div className="flex-1 flex flex-col gap-3 overflow-hidden min-h-0 relative">
        <div className="flex flex-col gap-4 shrink-0 px-1">
          <div className="flex items-center justify-between gap-4 min-h-[32px]">
            <ScrapingHeader title="Hasil Scrapping Pelunasan Piutang" lastUpdated={lastUpdated} scrapedPeriod={scrapedPeriod} activityLogTable="pelunasan_piutang" />
          ...
          <SearchAndReload searchQuery={searchQuery} setSearchQuery={setSearchQuery} onReload={() => setRefreshKey(prev => prev + 1)} loading={loading} placeholder="Cari faktur PP, ref. invoice, pelanggan..." />
        </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden relative">
          ...
          <TableFooter totalCount={totalCount} currentCount={data?.length || 0} label="Pelunasan Piutang" selectedCount={selectedIds.size} onClearSelection={clearSelection} loadTime={loadTime} />
        </div>
      </div>
