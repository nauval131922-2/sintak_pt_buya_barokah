...
export default function PRClient() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('pr_columnWidths');
      return saved ? JSON.parse(saved) : {
        id: 80, faktur: 220, tgl: 140, status: 120, username: 120, recid: 80
      };
    }
    return {};
  });
  ...
  useEffect(() => {
    localStorage.setItem('pr_columnWidths', JSON.stringify(columnWidths));
  }, [columnWidths]);
        ...
        if (active) {
          setData(prev => {
            const processData = (items: any[]) => (items || []).map((item: any) => {
              ...
              if (item.mydata) { try { parsedMyData = typeof item.mydata === 'string' ? JSON.parse(item.mydata) : item.mydata; } catch(e){} }
  ...
  const handleFetch = async () => {
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-pr?start=${chunk.start}&end=${chunk.end}&metaStart=${startStr}&metaEnd=${endStr}&silent=true`);
        if (res.ok) {
    ...
    try {
      ...
      if (successCount > 0) {
        persistScraperPeriod({ stateKey: 'prReportState', periodKey: 'PRClient_scrapedPeriod' }, startDate, endDate);
        setRefreshKey(prev => prev + 1);
        localStorage.setItem('sintak_data_updated', Date.now().toString());
        fetch('/api/activity-log', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action_type: 'SCRAPE', table_name: 'purchase_requests', message: `Scrape purchase request berhasil: ${totalScraped} baris (${startStr} - ${endStr}).`, raw_data: JSON.stringify({ total: totalScraped, start: startStr, end: endStr }) }),
        }).catch(() => {});
        setDialog({ isOpen: true, type: 'success', title: 'Berhasil', message: `Berhasil menarik ${totalScraped} Purchase Request (PR).` });
      }
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }: any) => {
        ...
        const isClosed = val?.includes('CLOSE');
        return (
          <span className={`
    ...
    <div className="flex-1 min-h-0 flex flex-col gap-6 animate-in fade-in duration-700 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-3 overflow-hidden min-h-0 relative">
        <div className="flex flex-col gap-4 shrink-0 px-1">
          ...
          <SearchAndReload searchQuery={searchQuery} setSearchQuery={setSearchQuery} onReload={() => setRefreshKey(prev => prev + 1)} loading={loading} placeholder="Cari faktur PR, produksi, keterangan..." />
        </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden relative">
          ...
          <TableFooter totalCount={totalCount} currentCount={data?.length || 0} label="Purchase Request (PR)" selectedCount={selectedIds.size} onClearSelection={clearSelection} loadTime={loadTime} />
        </div>
      </div>
