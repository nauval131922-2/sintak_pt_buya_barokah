  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      ...
      return saved ? JSON.parse(saved) : {
        id: 80, faktur: 180, nama_prd: 350, tgl: 120, qty_order: 180, username: 110, recid: 80
      };
    }
    return {};
  });
    ...
    <div className="flex-1 min-h-0 flex flex-col gap-6 animate-in fade-in duration-700 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-3 overflow-hidden min-h-0 relative">
        <div className="flex flex-col gap-4 shrink-0 px-1">
          ...
          <SearchAndReload searchQuery={searchQuery} setSearchQuery={setSearchQuery} onReload={() => setRefreshKey(prev => prev + 1)} loading={loading} placeholder="Cari ID, faktur, produk, atau pelanggan..." />
        </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden relative">
          ...
          <TableFooter totalCount={totalCount} currentCount={data?.length || 0} label="Order Produksi" selectedCount={selectedIds.size} onClearSelection={clearSelection} loadTime={loadTime} />
        </div>
      </div>
