...
export default function BahanBakuClient() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('bahanBaku_columnWidths');
      return saved ? JSON.parse(saved) : {
        id: 100, faktur: 220, tgl: 140, nama_barang: 350, qty: 120, hp_total: 180, username: 140, recid: 80
      };
    }
    return {};
  });
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const res = await fetch(`/api/bahan-baku?${queryParams.toString()}`);
  ...
  const handleFetch = async () => {
    ...
    try {
      const concurrency = 10; const queue = [...chunks];
      ...
      if (successCount > 0) {
        ...
        fetch('/api/activity-log', {
          ...
          body: JSON.stringify({ action_type: 'SCRAPE', table_name: 'bahan_baku', message: `Scrape bahan baku berhasil: ${totalScraped} baris (${startStr} - ${endStr}).`, raw_data: JSON.stringify({ total: totalScraped, start: startStr, end: endStr }) }),
        }).catch(() => {});
        setDialog({ isOpen: true, type: 'success', title: 'Berhasil', message: `Berhasil menarik ${totalScraped} data BBB Produksi.` });
      }
    ...
    <div className="flex-1 min-h-0 flex flex-col gap-6 animate-in fade-in duration-500 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-3 overflow-hidden min-h-0 relative">
        <div className="flex flex-col gap-4 shrink-0 px-1">
          ...
          <SearchAndReload searchQuery={searchQuery} setSearchQuery={setSearchQuery} onReload={() => setRefreshKey(prev => prev + 1)} loading={loading} placeholder="Cari nomor faktur, nama bahan baku, atau keterangan..." />
        </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden relative">
          ...
          <TableFooter totalCount={totalCount} currentCount={data?.length || 0} label="BBB Produksi" selectedCount={selectedIds.size} onClearSelection={clearSelection} loadTime={loadTime} />
        </div>
      </div>
