...
export default function PengirimanClient() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>(() => {
    if (typeof window !== 'undefined') {
      ...
      return saved ? JSON.parse(saved) : {
        id: 80, tgl: 110, faktur: 200, kd_supir: 180, total_faktur: 80, username: 100
      };
    }
    return {};
  });
  ...
  const handleFetch = async () => {
    ...
    setError(''); setData([]); setPage(1); setIsBatching(true); setLoading(true); setBatchProgress(0); setSearchQuery('');
    ...
    const processChunk = async (chunk: any) => {
      try {
        const res = await fetch(`/api/scrape-pengiriman?start=${chunk.start}&end=${chunk.end}&metaStart=${startStr}&metaEnd=${endStr}&silent=true`);
        if (res.ok) {
    ...
    try {
      ...
      if (successCount > 0) {
        persistScraperPeriod({ stateKey: 'shState', periodKey: 'PengirimanClient_scrapedPeriod' }, startDate, endDate);
        setRefreshKey(prev => prev + 1);
        localStorage.setItem('sintak_data_updated', Date.now().toString());
        fetch('/api/activity-log', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action_type: 'SCRAPE', table_name: 'pengiriman', message: `Scrape pengiriman berhasil: ${totalScraped} baris (${startStr} - ${endStr}).`, raw_data: JSON.stringify({ total: totalScraped, start: startStr, end: endStr }) }),
        }).catch(() => {});
        setDialog({ isOpen: true, type: 'success', title: 'Berhasil', message: `Berhasil menarik ${totalScraped} data Pengiriman.` });
      }
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      header: 'Sopir',
      size: 180,
      cell: ({ getValue, row }: any) => <span className={`font-semibold ${row.getIsSelected() ? 'text-emerald-900' : 'text-gray-800'}`}>{String(getValue())}</span>
    },
    {
    ...
    <div className="flex-1 min-h-0 flex flex-col gap-6 animate-in fade-in duration-700 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-3 overflow-hidden min-h-0 relative">
        <div className="flex flex-col gap-4 shrink-0 px-1">
          ...
          <SearchAndReload searchQuery={searchQuery} setSearchQuery={setSearchQuery} onReload={() => setRefreshKey(prev => prev + 1)} loading={loading} placeholder="Cari faktur SJ, sopir, status, admin..." />
        </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden relative">
          ...
          <TableFooter totalCount={totalCount} currentCount={data?.length || 0} label="Pengiriman" selectedCount={selectedIds.size} onClearSelection={clearSelection} loadTime={loadTime} />
        </div>
      </div>
