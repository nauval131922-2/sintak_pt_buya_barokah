...
function MainContentInner({
  ...
  return (
    <div data-portal-root className="flex [zoom:0.90] w-[111.11vw] h-[111.11vh] md:[zoom:0.82] md:w-[121.95vw] md:h-[121.95vh] overflow-hidden bg-[var(--bg-deep)] min-[1920px]:[zoom:1] min-[1920px]:w-screen min-[1920px]:h-screen">
      <Sidebar user={user} permissions={permissions} />
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        {/* Header Fixed untuk halaman selain Laporan Pekerjaan (atau untuk Desktop & Tablet md ke atas) */}
        <div className={isLaporanPekerjaanPage ? 'hidden md:block' : 'block'}>
          {renderHeaderNode()}
        </div>
        
        {/* Main Content - internal scroll area */}
        <div id="main-content-scroll" className={`flex-1 min-h-0 flex flex-col min-w-0 ${isLaporanPekerjaanPage ? 'overflow-hidden laporan-pekerjaan-scroll' : 'overflow-y-auto'} custom-scrollbar bg-[var(--bg-deep)] px-4 xl:px-8 pt-2 xl:pt-3 ${isLaporanPekerjaanPage ? 'pb-20' : 'pb-16 xl:pb-6'}`}>
