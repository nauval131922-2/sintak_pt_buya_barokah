...
export async function initIndexing(database: { execute: (sql: string) => Promise<unknown> }) {
  ...
  const indexes = [
    ...
    // Expression index sopd: konversi DD-MM-YYYY → YYYY-MM-DD untuk filter/sort tanggal
    "CREATE INDEX IF NOT EXISTS idx_sopd_tgl_iso ON sopd(substr(tgl,7,4)||'-'||substr(tgl,4,2)||'-'||substr(tgl,1,2));",
    "CREATE INDEX IF NOT EXISTS idx_sopd_tgl ON sopd(tgl);",
    ...
    // idx_jurnal_main_active sengaja ikut di-drop: tak ada query yang memakainya sejak urutan default
    // dikembalikan persis seperti semula (klausa LIKE-Koordinasi tak bisa memakai index apa pun).
    "DROP INDEX IF EXISTS idx_jurnal_tgl_desc;",
    "DROP INDEX IF EXISTS idx_jurnal_main_active;",
