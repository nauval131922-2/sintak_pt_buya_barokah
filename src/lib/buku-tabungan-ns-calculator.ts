// ponytail: kalkulator dan master parameter Buku Tabungan Non Security 9 x 14,5 cm (14. Pricelist Buku Tabungan Non Security)
// Referensi: Pricelist Buku Tabungan Non Security.xlsx sheets HARGA JULI 2026 (BUKU TABUNGAN 9 x 14,5 Non Security 24 Hal) + Source BUKU TABUNGAN 9 x 14,5 cm - Non Security Ryobi.xlsm (Master Art Carton 260 gsm 16400 +5% + Print Inter 3500, HVS 70 gsm 15700 +5% + Ryobi, 6 lbr/24 hal, laminasi glossy 0.35/cm2 min 50k, finishing susun+lipat+jahit+pound+packing)
// Ukuran: 9 x 14,5 cm (tertutup) 1 Muka Full Colour, laminasi glossy, jahit + pound + susun + lipat + packing

export interface BukuTabunganNsMasterParams {
  // A. Bahan Kertas Cover Art Carton 260 gsm
  tarifKertasCoverKg: number; // default 16400 /kg
  ...
  tarifPoundPerPcs: number; // pound + jasa per pcs (≈460)
...
export type BukuTabunganNsVarianType = '24 Hal' | '32 Hal' | '48 Hal';

export const BUKU_TABUNGAN_NS_VARIANTS: BukuTabunganNsVarianType[] = ['24 Hal', '32 Hal', '48 Hal'];

// ponytail: coverPcsPerA3 = 4 cover per A3+ (BUKU!P7=4), leavesPerA3 = 6 lembar isi per A3+ naive (Excel 1 Naik Cetak 4 + 1 Plano 8 + Cutter 3 => efektif ~6/A3 naive) — upgrade ke plano-aware jika presisi dibutuhkan
export const BUKU_TABUNGAN_NS_CONFIG: Record<BukuTabunganNsVarianType, {
  w: number; h: number;
  ...
  '24 Hal': {
    ...
    description: '9 × 14,5 cm (tertutup) · Cover AC 260 gsm 1 Muka FC + Laminasi Glossy · Isi HVS 70 gsm 1 Warna Bolak-Balik 24 Hal (6 lbr) · Jahit + Pound + Susun Lipat',
  },
  '32 Hal': {
    w: 9, h: 14.5, coverPcsPerA3: 4, leavesPerA3: 6, jumlahHalaman: 32, jumlahLbr: 8,
    description: '9 × 14,5 cm (tertutup) · Cover AC 260 gsm 1 Muka FC + Laminasi Glossy · Isi HVS 70 gsm 1 Warna Bolak-Balik 32 Hal (8 lbr) · Jahit + Pound + Susun Lipat',
  },
  '48 Hal': {
    w: 9, h: 14.5, coverPcsPerA3: 4, leavesPerA3: 6, jumlahHalaman: 48, jumlahLbr: 12,
    description: '9 × 14,5 cm (tertutup) · Cover AC 260 gsm 1 Muka FC + Laminasi Glossy · Isi HVS 70 gsm 1 Warna Bolak-Balik 48 Hal (12 lbr) · Jahit + Pound + Susun Lipat',
  },
};
...
export interface BukuTabunganNsSimulatorInput {
  oplah: number;
  varian: BukuTabunganNsVarianType;
  marginPct: number;
...
export function calculateBukuTabunganNsHpp(
  ...
  // 1. Kebutuhan Cover A3+ & Biaya Kertas Cover AC 260 gsm
  ...
  add('Kertas Cover Art Carton 260 gsm', biayaKertasCover,
