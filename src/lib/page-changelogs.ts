...
export const PAGE_CHANGELOGS: Record<string, PageChangelog> = {
  ...
  'laporan-pekerjaan-2026-09-01': entry({
    ...
    items: [
      ...
      'feat: Filter rentang tanggal (Dari - Sampai) & jam pengerjaan',
      'feat: Atur ukuran font tabel presisi ala Excel (Input angka 6px-48px, dropdown preset, tombol - / +)',
      'feat: Akses tambah pekerjaan untuk anggota/operator',
      'fix: Otomatis mengisi PIC & Bagian saat input pekerjaan jika kolomnya disembunyikan lewat konfigurasi role',
      'fix: Filter status di tabel utama tidak memotong daftar pekerjaan di modal detail',
      'fix: Data tidak tersimpan ke database jika pengguna menutup modal tanpa menekan tombol Simpan',
      'fix: Lis warna status baris tabel menggunakan inset shadow agar tidak tembus keluar header saat scroll',
      'fix: Panel popup profil sidebar otomatis menutup (auto-collapse) saat sidebar menciut',
    ],
  }),
